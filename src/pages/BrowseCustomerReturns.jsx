...
function displayQtyType(t, custom) {
  return t === "أخرى / Other" ? (custom || "") : (t || "");
}
function isKgType(t) {
        ...
        <div style={viewerThumbsWrap}>
          {images.map((src, i) => (
            <button key={i} style={viewerThumbTile} onClick={() => setPreview(src)} title={`Image ${i + 1}`}>
              <img src={src} alt={`thumb-${i}`} style={viewerThumbImg} loading="lazy" />
            </button>
          ))}
        </div>
      </div>
...
export default function BrowseCustomerReturns() {
  ...
  const [onlyKgType, setOnlyKgType] = useState(false);
  const [onlyExpired, setOnlyExpired] = useState(false);
  const [onlyChangedAction, setOnlyChangedAction] = useState(false);

  /* ===== Read-only Image Viewer State (MOVED UP) ===== */
  const [viewerOpen, setViewerOpen] = useState(false);
  const [viewerTitle, setViewerTitle] = useState("");
  const [viewerImages, setViewerImages] = useState([]);
  function openViewer(title, images) {
  ...
  /* selected report (for 'day' scope) */
  const selectedReport = useMemo(
    () => filteredReportsAsc.find((r) => r.reportDate === selectedDate) || null,
    [filteredReportsAsc, selectedDate]
  );

  /* changeMap current day (for 'day' scope) */
  ...
  /* ====== Search scope: flatten all items in range ====== */
  const flatItemsInRange = useMemo(() => {
    const out = [];
    for (const rep of filteredReportsAsc) {
      const date = rep.reportDate;
      const cmap = changeMapByDate.get(date) || new Map();
      (rep.items || []).forEach((row) => {
        const k = itemKey(row);
        const ch = cmap.get(k) || null;
        out.push({ date, row, change: ch, key: k });
      });
    }
    return out;
  }, [filteredReportsAsc, changeMapByDate]);

  /* ====== build filter options (customers/actions from range) ====== */
  const allCustomers = useMemo(() => {
    const s = new Set();
    flatItemsInRange.forEach(({ row }) => {
      const v = String(row?.customerName || "").trim();
      if (v) s.add(v);
    });
  ...
  /* ====== smart filter function used for both scopes ====== */
  const matchesSmart = (date, row, ch) => {
    const txt = (q || "").trim().toLowerCase();
    const hay = [
      row?.productName,
      row?.origin,
      row?.customerName,
      displayQtyType(row?.qtyType, row?.customQtyType),
      row?.expiry,
      row?.remarks,
      actionText(row),
      date,
    ].map((x) => String(x || "").toLowerCase());
    if (txt && !hay.some((s) => s.includes(txt))) return false;

    if (filterCustomer && String(row?.customerName || "") !== filterCustomer) return false;
    if (filterAction && String(actionText(row) || "") !== filterAction) return false;

    if (onlyWithImages && !(Array.isArray(row?.images) && row.images.length > 0)) return false;
    if (onlyKgType && !isKgType(row?.qtyType)) return false;

    if (onlyExpired) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const e = row?.expiry ? new Date(row.expiry) : null;
      if (!(e && e.getTime() < today.getTime())) return false;
    }

    if (onlyChangedAction) {
      const curr = actionText(row);
      if (!(ch && ch.to === curr)) return false;
    }

    return true;
  };

  /* ====== filtered results for 'all' and 'day' ====== */
  const filteredAllResults = useMemo(() => {
    return flatItemsInRange.filter(({ date, row, change }) => matchesSmart(date, row, change));
  }, [flatItemsInRange, q, filterCustomer, filterAction, onlyWithImages, onlyKgType, onlyExpired, onlyChangedAction]);

  const dayRows = useMemo(() => {
    const rows = selectedReport?.items || [];
    return rows
      .map((row) => ({ date: selectedReport?.reportDate || "", row, change: changeMap.get(itemKey(row)) || null }))
      .filter(({ date, row, change }) => matchesSmart(date, row, change));
  }, [selectedReport, changeMap, q, filterCustomer, filterAction, onlyWithImages, onlyKgType, onlyExpired, onlyChangedAction]);

  /* ====== Export helpers ====== */
  async function ensureJsPDF() {
    if (_pdfLoaded && window.jspdf?.jsPDF) return window.jspdf.jsPDF;
    if (window.jspdf?.jsPDF) { _pdfLoaded = true; return window.jspdf.jsPDF; }
  ...
  async function ensureAutoTable() {
    if (_autoTableLoaded || window.jspdf?.jsPDF?.API?.autoTable) { _autoTableLoaded = true; return; }
    await new Promise((resolve, reject) => {
      const s = document.createElement("script");
    ...
    _autoTableLoaded = true;
  }

  // XLSX loader & export
  async function ensureXLSX() {
    if (_xlsxLoaded && window.XLSX) return window.XLSX;
    if (window.XLSX) { _xlsxLoaded = true; return window.XLSX; }
    await new Promise((resolve, reject) => {
      const s = document.createElement("script");
  ...
  async function exportXLSX(rows, filename = "customer_returns_search_results.xlsx") {
    const XLSX = await ensureXLSX();

    const header = ["DATE","PRODUCT","ORIGIN","CUSTOMER","QTY","QTY TYPE","EXPIRY","REMARKS","ACTION"];
    const data = [header];

    rows.forEach(({ date, row, change }) => {
      const qtyType = displayQtyType(row?.qtyType, row?.customQtyType);
      let act = actionText(row) || "";
      if (change && change.to === actionText(row)) {
        const t = toTs(change.at);
        const d = t ? new Date(t) : null;
        const dd = d ? String(d.getDate()).padStart(2, "0") : "";
        const mm = d ? String(d.getMonth() + 1).padStart(2, "0") : "";
        const yyyy = d ? d.getFullYear() : "";
        const dateTxt = d ? ` ${dd}/${mm}/${yyyy}` : "";
        act = `${(change.from || "").trim()} -> ${(change.to || "").trim()}${dateTxt}`;
      }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const head = [["SL", "PRODUCT", "ORIGIN", "CUSTOMER", "QTY", "QTY TYPE", "EXPIRY", "REMARKS", "ACTION"]];
      const body = dayRows.map(({ row }, i) => {
        const qtyType = displayQtyType(row.qtyType, row.customQtyType);
        const curr = actionText(row);
        const k = itemKey(row);
  ...
  return (
    <>
      ...
      <div style={pageWrap}>
        ...
        <div style={hero}>
          <div>
            ...
            <div style={{ fontWeight: 600, fontSize: ".95rem", color: "#0f172a", opacity: .85 }}>
              KPIs, date filter, smart search — per-day or across all reports in range.
            </div>
          </div>
          <div style={brandWrap}>
        ...
        >
          <DonutCard percent={100} centerText={String(kpi.totalItems)} label="Total returned items" subLabel="Across selected range" color="#059669" />
          <DonutCard percent={100} centerText={String(kpi.totalQtyKg)} label="Returned weight (kg)" subLabel="Weight-based items only" color="#1d4ed8" />
          <DonutCard percent={100} centerText={String(kpi.totalReports)} label="Total reports" subLabel="Number of days" color="#0ea5e9" />
              ...
              <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
                {kpi.topProducts.map((p, i) => (
                  <li key={i} style={listRow}>
                    ...
                    <span style={{ fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={p.name}>
                      {p.name}
                    </span>
        ...
        {serverErr && (
          <div style={{ textAlign: "center", marginBottom: 10, color: "#b91c1c", fontWeight: 700 }}>
            {serverErr} <button onClick={reload} style={{ marginLeft: 8, border: "none", background: "#111827", color: "#fff", borderRadius: 8, padding: "4px 10px", cursor: "pointer" }}>Retry</button>
          </div>
        )}
        ...
        >
          <div style={{ display: "flex", gap: 12, justifyContent: "center", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <label>From:
              <input type="date" value={filterFrom} onChange={(e) => setFilterFrom(e.target.value)} style={{ ...dateInputStyle, marginLeft: 8 }} />
            </label>
            <label>To:
              <input type="date" value={filterTo} onChange={(e) => setFilterTo(e.target.value)} style={{ ...dateInputStyle, marginLeft: 8 }} />
            </label>
            {(filterFrom || filterTo) && (
              <button onClick={() => { setFilterFrom(""); setFilterTo(""); }} style={clearBtn}>🧹 Clear filter</button>
            )}
          </div>

          {/* Smart search row */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 160px 160px 140px 140px 140px 180px", gap: 10, marginTop: 12 }}>
            <input
              placeholder="🔎 Search text (product/origin/customer/qty type/remarks/expiry/action/date)…"
              value={q}
              onChange={(e) => setQ(e.target.value)}
              style={{ ...dateInputStyle, width: "100%" }}
            />
            <select value={filterCustomer} onChange={(e) => setFilterCustomer(e.target.value)} style={dateInputStyle}>
              <option value="">All customers</option>
              {allCustomers.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
            <select value={filterAction} onChange={(e) => setFilterAction(e.target.value)} style={dateInputStyle}>
            ...
            <label style={{ display:"flex", alignItems:"center", gap:8 }}>
              <input type="checkbox" checked={onlyKgType} onChange={(e)=>setOnlyKgType(e.target.checked)} />
              KG type
            </label>
            <label style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <label style={{ display:"flex", alignItems:"center", gap:8 }}>
              <input type="checkbox" checked={onlyChangedAction} onChange={(e)=>setOnlyChangedAction(e.target.checked)} />
              Changed action
            </label>
          </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 10, alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ fontWeight: 800, color: "#0f172a" }}>Search scope:</div>
            <select value={scope} onChange={(e)=>setScope(e.target.value)} style={{ ...dateInputStyle, minWidth: 220 }}>
              <option value="day">Selected Day (right panel)</option>
              <option value="all">All Reports (Range)</option>
            </select>

            {scope === "all" ? (
              <>
                <div style={{ fontWeight: 700, color: "#0f172a" }}>Results: {filteredAllResults.length}</div>
                <button
                  onClick={() => exportXLSX(filteredAllResults, "customer_returns_search_results.xlsx")}
                  style={{ background: "#111827", color: "#fff", border: "none", borderRadius: 12, padding: "9px 14px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 10px 22px rgba(17,24,39,.25)" }}
                  title="Export filtered results to Excel"
                >
                  ⬇️ Export Excel (results)
                </button>
              </>
            ) : (
              <>
                <div style={{ fontWeight: 700, color: "#0f172a" }}>Shown: {dayRows.length}</div>
                <button
                  onClick={handleExportPDF}
                  style={{ background: "#111827", color: "#fff", border: "none", borderRadius: 12, padding: "9px 14px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 10px 22px rgba(17,24,39,.25)" }}
                  title="Export current day to PDF"
                >
                  ⬇️ Export PDF (day)
                </button>
              </>
        ...
        {/* Layout: left tree + right details OR unified results */}
        <div style={{ display: "flex", alignItems: "flex-start", gap: 16, minHeight: 420 }}>
          {/* Left: date tree (kept for navigation) */}
          <div style={leftTree}>
          ...
          {/* Right: either unified results (all) or selected day */}
          <div style={rightPanel}>
            {scope === "all" ? (
              <>
                <div style={{ fontWeight: "bold", color: "#0f172a", fontSize: "1.15em", marginBottom: 8 }}>
                  Search results across all reports in range ({filteredAllResults.length})
                </div>
                <table style={table}>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {filteredAllResults.map(({ date, row, change }, i) => {
                      const curr = actionText(row);
                      const showChange = change && change.to === curr;
                      return (
                        <tr key={i}>
                          <td style={td}>{i + 1}</td>
                          <td style={td}>{date}</td>
                          <td style={td}>
                            ...
                            {Array.isArray(row.images) && row.images.length > 0 && (
                              <button style={viewImgBtn} onClick={() => openViewer(row.productName || "", row.images)} title="View images">
                                View ({row.images.length})
                              </button>
                            )}
                          </td>
                          ...
                          <td style={td}>{displayQtyType(row.qtyType, row.customQtyType)}</td>
                          ...
                          <td style={{ ...td, background: showChange ? "#e9fce9" : td.background }}>
                            {showChange ? (
                              <div style={{ lineHeight: 1.2 }}>
                                <div>
                                  <span style={{ opacity: 0.8 }}>{change.from}</span>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    <div style={{ fontWeight: "bold", color: "#0f172a", fontSize: "1.15em" }}>
                      Returns details ({selectedReport.reportDate}) — Shown: {dayRows.length}
                    </div>
                    <button
                      onClick={handleExportPDF}
                      style={{ background: "#111827", color: "#fff", border: "none", borderRadius: 12, padding: "9px 14px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 10px 22px rgba(17,24,39,.25)" }}
