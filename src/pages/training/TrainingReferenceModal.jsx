...
M) Trolleys — pre-use check before every shift.
   م) العربات (الترولي) — الفحص قبل الاستخدام في كل وردية.
   CHECK BEFORE USE: All wheels/castors roll and swivel freely — no seized, wobbling, or flat-spotted wheel. No cracked, bent, or missing parts (handle, frame, welds, shelves). Handle grips intact and not slippery. Brakes (if fitted) engage and release properly. Trolley is clean — no grease or product residue on the handle or deck. IF ANY DEFECT: Tag the trolley OUT OF SERVICE, remove it from the area, and raise a maintenance request. Never "make do" with a damaged trolley — a seized wheel is the single most common cause of trolley strain injuries.
   افحص قبل الاستخدام: كل العجلات تدور وتلتف بحرية — لا عجلة عالقة أو مهتزّة. لا أجزاء مكسورة أو مفقودة (المقبض، الهيكل، اللحامات، الأرفف). المقابض سليمة وغير زلقة. الفرامل (إن وُجدت) تعمل. الترولي نظيف. عند أي عطل: ضع بطاقة "خارج الخدمة"، أخرجه من المنطقة، وافتح طلب صيانة. لا تستخدم ترولي تالف — العجلة العالقة هي السبب الأول لإصابات الترولي.
N) Loading a trolley — capacity, balance, and stability.
   ن) تحميل العربة — السعة والتوازن والاستقرار.
   RULES: Never exceed the trolley's rated capacity — it must be marked on the trolley (carcass trolleys are rated 300–400 kg per SSOP-EQS-07 Annex A; general-purpose trolleys are far lower). If no rating is marked, stop and ask the supervisor — do not guess. Heavy items on the bottom deck, light on top. Load evenly over the deck — never all the weight on one side or overhanging the edge. Load must not block your line of sight over or around the trolley. Secure loose or tall loads (strap, stretch wrap, or cage trolley). BEFORE MOVING: Push the load gently — if it rocks or shifts, re-stack it. An unstable load on a moving trolley is a falling-object hazard for everyone in the aisle.
   القواعد: لا تتجاوز السعة المصمّمة للترولي — ويجب أن تكون موسومة عليه (عربات الذبائح مصنّفة 300–400 كغم حسب SSOP-EQS-07 الملحق أ، والعربات العامة أقل بكثير). إذا لم تكن السعة موسومة، توقف واسأل المشرف — لا تخمّن. الثقيل في الرف السفلي والخفيف في الأعلى. وزّع الحمل بالتساوي — لا كل الوزن على جهة واحدة ولا خارج حدود السطح. الحمل يجب ألا يحجب الرؤية. ثبّت الأحمال العالية أو غير المربوطة. قبل التحريك: ادفع الحمل برفق — إذا اهتزّ أعد ترتيبه.
O) Moving a trolley — push, don't pull.
   س) تحريك العربة — ادفع ولا تسحب.
   PUSH: Always push a loaded trolley — do not pull it. Pushing lets you use body weight and leg muscles, keeps the back neutral, and keeps the load in front of you where you can see it. Pulling twists the spine, puts the load behind you (out of sight), and risks the trolley running over your heels. BOTH HANDS on the handle at all times — one-handed steering loses control and encourages twisting. POSTURE: Back straight, lean into the push from the legs, elbows slightly bent, shoulders square to the direction of travel. EXCEPTION: Pull only briefly to reposition in a tight space or through a doorway — then return to pushing.
   ادفع: ادفع الترولي المحمّل دائماً ولا تسحبه. الدفع يتيح استخدام وزن الجسم وعضلات الساقين ويبقي الظهر مستقيماً والحمل أمامك ضمن مجال الرؤية. السحب يلوي العمود الفقري ويضع الحمل خلفك ويعرّض كعبيك للدهس. استخدم اليدين معاً على المقبض دائماً — التوجيه بيد واحدة يفقد السيطرة. الوضعية: الظهر مستقيم، ميل للأمام من الساقين، الكتفان باتجاه الحركة. استثناء: اسحب لثوانٍ فقط للمناورة في مساحة ضيقة ثم عُد للدفع.
P) Speed, turning, ramps, and stopping.
   ع) السرعة والدوران والمنحدرات والتوقف.
   SPEED: Walking pace only — never run or jog with a trolley. You must be able to stop within one step. TURNING: Slow down before the turn and make it wide and gradual — sharp turns tip the load. Approach blind corners and doorways at reduced speed and sound a verbal warning ("coming through"). STOPPING: No sudden stops — decelerate gradually or the load keeps moving and topples. RAMPS AND SLOPES: Going UP — push with both hands, never let go; use two people for heavy loads. Going DOWN — stay on the UPHILL side and control the descent; never let the trolley run ahead of you. Never leave a trolley unattended on any slope. FLOOR CONDITION: Slow right down on wet floors, thresholds, drain covers, and expansion joints — these catch small castors and stop the trolley dead while the load keeps going.
   السرعة: سرعة المشي فقط — لا ركض. يجب أن تكون قادراً على التوقف خلال خطوة واحدة. الدوران: خفّف السرعة قبل المنعطف واجعله واسعاً وتدريجياً — المنعطفات الحادة تقلب الحمل. اقترب من الزوايا العمياء والأبواب بسرعة منخفضة مع تنبيه صوتي. التوقف: لا توقف مفاجئ — خفّف تدريجياً. المنحدرات: عند الصعود ادفع بيدين ولا تترك المقبض واستخدم شخصين للأحمال الثقيلة. عند النزول قف في الجهة العليا وتحكم بالنزول — لا تدع الترولي يسبقك. لا تترك ترولي على منحدر دون مراقبة. الأرضية: خفّف كثيراً على الأرضيات الرطبة وأغطية الصرف والفواصل — تعلق فيها العجلات الصغيرة فيتوقف الترولي ويستمر الحمل.
Q) Brakes and parking.
   ف) الفرامل وإيقاف العربة.
   ENGAGE BRAKES: Whenever the trolley is stationary — during loading, unloading, waiting, or any pause. Always on any slope, even a slight one. Always before you step away from it, even for a few seconds. LOCK THE WHEELS before loading or unloading heavy items — a trolley that rolls away mid-lift causes the worst back injuries. NO BRAKES FITTED? Park against a wall or rack, never in the middle of an aisle, and chock the wheels on any incline. PARKING RULES: Never leave a trolley blocking an aisle, fire exit, fire extinguisher, electrical panel, or doorway. Return trolleys to the designated parking bay after use.
   فعّل الفرامل: كلما توقّف الترولي — أثناء التحميل أو التفريغ أو الانتظار أو أي توقف. دائماً على أي منحدر ولو بسيط. دائماً قبل الابتعاد عنه ولو لثوانٍ. اقفل العجلات قبل تحميل أو تفريغ الأصناف الثقيلة — الترولي الذي يتدحرج أثناء الرفع يسبب أسوأ إصابات الظهر. لا فرامل؟ أوقفه بمحاذاة جدار أو رف ولا تتركه وسط الممر، وضع مسند عجلات على أي ميل. قواعد الإيقاف: لا تترك ترولي يسدّ ممراً أو مخرج طوارئ أو طفاية أو لوحة كهرباء أو باباً. أعِد الترولي إلى مكان الإيقاف المخصص بعد الاستخدام.
   References: UAE OHS Federal Law No. 8/1980 | Dubai Municipality H&S Code | ISO 45001:2018 | HSE Manual Handling Regulations | SOP-OHS-06 (Safe Manual Lifting & Trolley Handling).`,

  "OHS: Loading & Unloading": `A) Why loading and unloading is one of the highest-risk activities on site.
   أ) لماذا يُعد التحميل والتفريغ من أخطر الأنشطة في الموقع.
   FACTS: The loading bay is the only place where people, moving vehicles, heavy loads, height and weather meet at the same time. Injuries here are rarely minor — being struck, crushed or falling from a trailer causes fractures, disability or death. Most incidents happen in the first and last five minutes of the job, when the vehicle is arriving or about to leave.
   حقائق: منطقة التحميل هي المكان الوحيد الذي يجتمع فيه البشر والمركبات المتحركة والأحمال الثقيلة والارتفاع والطقس في وقت واحد. الإصابات هنا نادراً ما تكون بسيطة — الدهس أو السحق أو السقوط من المقطورة يسبب كسوراً أو عجزاً أو وفاة. معظم الحوادث تقع في أول وآخر خمس دقائق من العملية، عند وصول المركبة أو قبيل مغادرتها.
   STANDARD: ISO 45001:2018 | UAE Federal Decree-Law 33/2021 | on-site traffic management requirements | SOP-OHS-11.
B) Before work starts — securing the vehicle.
   ب) قبل بدء العمل — تأمين المركبة.
   SEQUENCE: 1) Driver reports to the shift supervisor and is told which dock to use and the 10 km/h site speed limit. 2) Engine off. 3) Parking brake applied. 4) Wheel chocks fitted on both sides. 5) The vehicle key is handed to the supervisor and stays with them until the job is finished (key control) — this is what stops a vehicle driving off while someone is still inside it. 6) A short toolbox talk covering the load, its hazards and who does what, with attendance signed.
   التسلسل: 1) يراجع السائق مشرف المناوبة ويُبلَّغ برصيف التحميل وبالسرعة القصوى 10 كم/س. 2) إطفاء المحرك. 3) تفعيل فرامل الوقوف. 4) تركيب أوتاد العجلات على الجانبين. 5) تسليم مفتاح المركبة للمشرف ويبقى لديه حتى انتهاء العملية (ضبط المفتاح) — وهذا ما يمنع تحرك المركبة بينما لا يزال أحد داخلها. 6) اجتماع Toolbox قصير يشرح الحمولة ومخاطرها وتوزيع المهام مع توقيع الحضور.
C) Traffic management — keeping people away from moving vehicles.
   ج) إدارة الحركة — إبعاد الناس عن المركبات المتحركة.
   CONTROLS: A painted pedestrian walkway physically separated from the vehicle route. One-way flow through the yard. 10 km/h maximum. Convex mirrors at blind corners. A trained banksman for EVERY reversing manoeuvre, standing where the driver can always see them and using agreed hand signals. RULE: if the driver loses sight of the banksman, the driver stops immediately.
   الضوابط: ممر مشاة مدهون ومفصول بحواجز عن مسار المركبات. اتجاه سير واحد داخل الساحة. سرعة قصوى 10 كم/س. مرايا محدبة عند الزوايا العمياء. مراقب مدرّب عند كل مناورة رجوع للخلف، يقف حيث يراه السائق دائماً ويستخدم إشارات متفق عليها. القاعدة: إذا غاب المراقب عن نظر السائق، يتوقف السائق فوراً.
D) Crush prevention — the exclusion zone.
   د) منع السحق — منطقة الاستبعاد.
   THE HAZARD: Being caught between the vehicle and the dock, the racking or a fixed object. This is the injury most likely to be fatal. CONTROLS: A painted exclusion zone behind and beside the vehicle that must stay completely clear while it is moving. Rubber dock bumpers. A red/green light signal between the driver and the dock — green means the dock is clear, red means do not move. Never walk between a reversing vehicle and a fixed structure, and never rely on the driver having seen you.
   الخطر: الانحشار بين المركبة والرصيف أو الرفوف أو جسم ثابت، وهو أكثر الإصابات احتمالاً للوفاة. الضوابط: منطقة استبعاد مدهونة خلف المركبة وجانبيها تبقى خالية تماماً أثناء حركتها. صادّات مطاطية على الرصيف. إشارة ضوئية أحمر/أخضر بين السائق والرصيف — الأخضر يعني الرصيف خالٍ والأحمر يعني لا تتحرك. لا تمشِ أبداً بين مركبة ترجع للخلف وجسم ثابت، ولا تفترض أن السائق رآك.
E) Fall prevention — never jump down.
   هـ) منع السقوط — لا تقفز أبداً.
   CONTROLS: Fixed steps or an approved access platform for getting on and off the vehicle — jumping from a trailer bed is the single most common cause of ankle and knee injuries in this task. A guardrail at the dock edge and a safety chain across the loading opening whenever no vehicle is docked. Climbing on stacked loads is prohibited. A harness is required for any work above 1.8 m. Edge-warning signage and floor marking at the dock lip.
   الضوابط: سلالم ثابتة أو منصة وصول معتمدة للصعود والنزول — القفز من صندوق المقطورة هو السبب الأول لإصابات الكاحل والركبة في هذه المهمة. حاجز واقٍ على حافة الرصيف وسلسلة أمان على فتحة التحميل كلما لم تكن هناك مركبة راسية. يُمنع الصعود فوق الأحمال المستيفة. حزام أمان إلزامي لأي عمل يتجاوز 1.8 متر. لافتات تحذير من الحافة وشريط أرضي عند حرف الرصيف.
F) Manual handling inside the bay.
   و) المناولة اليدوية داخل منطقة التحميل.
   RULES: 25 kg is the limit for one person; anything heavier is a two-person lift on one call, or a mechanical aid. Trolleys, pallet trucks and the dock ramp must be available before the job starts — an unavailable trolley is never a reason to lift by hand, the job waits. Keep the load close to the body, turn with the feet, and never twist under load. Heavy items must be weight-marked.
   القواعد: 25 كجم حد الفرد الواحد؛ وما فوق ذلك رفع ثنائي بإشارة واحدة أو بوسيلة ميكانيكية. يجب توفر العربات ومكابس البالات والمنحدر قبل بدء العمل — وتعطّل العربة ليس مبرراً للرفع اليدوي بل تنتظر العملية. أبقِ الحمل قريباً من الجسم، وأدِر قدميك ولا تلتفّ بجذعك تحت الحمل. الأصناف الثقيلة تُعلَّم بوزنها.
G) Load securement before departure.
   ز) تثبيت الحمولة قبل المغادرة.
   CONTROLS: Only approved straps and load bars, inspected before every use — any strap that is cut, frayed or has a damaged ratchet is withdrawn from service immediately, not taped or reused. Respect the maximum stacking height and distribute weight evenly. Complete the pre-departure securement checklist and have it signed by BOTH the driver and the supervisor. The chocks come off and the key is returned only after the doors are closed and everyone is clear of the manoeuvring area.
   الضوابط: أحزمة وقضبان تثبيت معتمدة فقط، تُفحص قبل كل استخدام — وأي حزام مقطوع أو متآكل أو بمشدّ تالف يُسحب من الخدمة فوراً ولا يُلصق ولا يُعاد استخدامه. التزم بحد ارتفاع التستيف ووزّع الوزن بالتساوي. عبّئ قائمة فحص التثبيت قبل المغادرة ووقّعها من السائق والمشرف معاً. لا تُرفع الأوتاد ولا يُعاد المفتاح إلا بعد إغلاق الأبواب وخروج الجميع من منطقة المناورة.
H) Visibility, lighting and weather — when to stop.
   ح) الرؤية والإضاءة والطقس — متى تتوقف.
   CONTROLS: At least 200 lux in the loading area, verified by a documented monthly check. A high-visibility vest is mandatory for everyone entering the bay, with no exceptions for visitors or drivers. Dock lights directed into the trailer. Audible and visual reversing alarms on every vehicle and forklift. Anti-slip surfacing and a canopy over the dock, with immediate drying and wet-floor signage. STOP-WORK: the shift supervisor has stated authority to suspend loading in heavy rain, on an iced floor, or when lighting fails — and work resumes only after the condition is corrected, never on a verbal assurance.
   الضوابط: 200 لكس على الأقل في منطقة التحميل مع فحص شهري موثّق. السترة العاكسة إلزامية لكل من يدخل المنطقة بلا استثناء للزوار أو السائقين. مصابيح رصيف موجّهة إلى داخل الشاحنة. إنذار رجوع صوتي وضوئي على كل مركبة ورافعة. سطح مانع للانزلاق ومظلة فوق الرصيف مع تجفيف فوري ولافتات أرضية مبللة. إيقاف العمل: لمشرف المناوبة صلاحية معلنة بتعليق التحميل عند الأمطار الغزيرة أو تجمّد الأرضية أو تعطّل الإضاءة، ولا يُستأنف العمل إلا بعد معالجة السبب، لا بتطمين شفهي.
I) When something goes wrong — reporting and stop-work authority.
   ط) عند وقوع خطأ — الإبلاغ وصلاحية إيقاف العمل.
   RULE: Every worker in the bay has the authority to stop the operation if they see an unsafe condition, and no one is penalised for using it. Every incident AND every near-miss is reported the same day under SOP-OHS-09, even when nobody was hurt — a near-miss is a free warning that the next one may be a fatality. Report a missing chock, a failed reversing alarm, a damaged strap or a burnt-out dock light immediately; these are the small failures that precede the serious injuries.
   القاعدة: لكل عامل في المنطقة صلاحية إيقاف العملية إذا رأى وضعاً غير آمن، ولا يُحاسَب أحد على استخدامها. كل حادث وكل شبه حادث يُبلَّغ عنه في اليوم نفسه وفق SOP-OHS-09، حتى لو لم يُصب أحد — فشبه الحادث إنذار مجاني بأن التالي قد يكون وفاة. أبلغ فوراً عن وتد مفقود أو إنذار رجوع معطّل أو حزام تالف أو مصباح رصيف محترق؛ فهذه هي الأعطال الصغيرة التي تسبق الإصابات الجسيمة.`,
...
M) The emergency response chain at site — who to call, in what order.
   م) سلسلة الاستجابة للطوارئ في الموقع — من تتصل به وبأي ترتيب.
   STEP 1: STAY CALM. Do not crowd the casualty. STEP 2: Call the designated FIRST AIDER on shift and the HSE OFFICER immediately — their names and numbers are on the F-31 Emergency Contacts list posted in every area. STEP 3: The first aider / HSE officer assesses severity. MINOR → first aid treatment at the company clinic (QA Laboratory). MAJOR OR SEVERE → call the ambulance IMMEDIATELY on 998 and do not move the casualty unless there is ongoing danger. STEP 4: One person is assigned to meet the ambulance at the gate and guide it in. STEP 5: The HSE Officer investigates and completes the F-01 Incident Report. CRITICAL NUMBER CHECK: In the UAE, AMBULANCE = 998. Police = 999. Civil Defence (fire) = 997. Unified emergency = 112. Do not confuse 998 and 999 — dialling police for a medical emergency costs minutes.
   الخطوة 1: ابقَ هادئاً ولا تتجمهر حول المصاب. الخطوة 2: اتصل فوراً بالمسعف المناوب وضابط HSE — أسماؤهم وأرقامهم على قائمة F-31 المعلّقة في كل منطقة. الخطوة 3: المسعف/ضابط HSE يقيّم الشدة. حالة بسيطة ← إسعاف في عيادة الشركة (مختبر QA). حالة خطيرة ← اتصل بالإسعاف فوراً 998 ولا تحرّك المصاب إلا عند وجود خطر مستمر. الخطوة 4: تعيين شخص لاستقبال سيارة الإسعاف عند البوابة وإرشادها. الخطوة 5: ضابط HSE يحقق ويعبّئ نموذج F-01. تنبيه مهم: في الإمارات الإسعاف = 998، الشرطة = 999، الدفاع المدني = 997، الطوارئ الموحد = 112. لا تخلط بين 998 و999.
N) The Primary Survey — DRABC.
   ن) الفحص الأولي — DRABC.
   WHAT IT IS: A systematic order for approaching ANY casualty — from a cut finger to a cardiac arrest. Life-threatening problems are found and dealt with FIRST, in this order. Never skip a letter. D = DANGER. R = RESPONSE. A = AIRWAY. B = BREATHING. C = COMPRESSIONS (CPR). D — DANGER: Eliminate or minimise the danger BEFORE you approach. Check for: live electricity, moving machinery, chemical spill, ammonia leak, unstable stacks, traffic. Ensure the safety of YOURSELF first, then bystanders, then the casualty. RULE: A dead rescuer helps no one. If it is too dangerous to approach — keep a safe distance and call the emergency services. Move the casualty ONLY if absolutely necessary. Use bystanders — one to call 998, one to control access, one to fetch the first aid kit and the first aider.
   ما هو: ترتيب منهجي للتعامل مع أي مصاب — من جرح بسيط إلى توقف قلبي. المشاكل المهددة للحياة تُكتشف وتُعالج أولاً بهذا الترتيب. لا تتخطَّ أي حرف. D خطر / R استجابة / A مجرى هواء / B تنفس / C ضغطات. D — الخطر: أزل أو قلّل الخطر قبل الاقتراب. تحقق من: كهرباء حية، آلات متحركة، انسكاب كيميائي، تسرب أمونيا، رصّات غير مستقرة، حركة مركبات. أمّن نفسك أولاً ثم المارة ثم المصاب. القاعدة: المنقذ المصاب لا ينفع أحداً. إذا كان الاقتراب خطراً جداً — ابقَ على مسافة آمنة واتصل بالطوارئ. لا تحرّك المصاب إلا للضرورة القصوى. استعن بالموجودين — واحد يتصل 998، واحد يمنع الازدحام، واحد يحضر حقيبة الإسعاف والمسعف.
O) R = Response — the COWS check.
   س) R = الاستجابة — فحص COWS.
   METHOD: Touch and talk. NEVER SHAKE an unconscious casualty — you can worsen a spinal injury. Tap the shoulders gently and speak loudly and clearly. COWS: "Can you hear me?" — "Open your eyes" — "What's your name?" — "Squeeze my hand". IF RESPONSIVE: The airway is open. Ask what happened and what they feel. Treat what you find. Call an ambulance if needed. Keep monitoring — a responsive casualty can deteriorate. IF UNRESPONSIVE: Shout for help, call 998 IMMEDIATELY (or send someone), and move straight to AIRWAY. Do not leave the casualty alone to make the call if anyone else is available.
   الطريقة: المس وتكلّم. لا تهزّ فاقد الوعي أبداً — قد تفاقم إصابة العمود الفقري. اربت على الكتفين برفق وتكلم بصوت عالٍ وواضح. COWS: "هل تسمعني؟" — "افتح عينيك" — "ما اسمك؟" — "اضغط على يدي". إذا استجاب: مجرى الهواء مفتوح. اسأله ماذا حدث وبماذا يشعر. عالج ما تجده. اتصل بالإسعاف عند اللزوم. استمر بالمراقبة — الحالة قد تتدهور. إذا لم يستجب: نادِ للمساعدة، اتصل 998 فوراً (أو أرسل شخصاً)، وانتقل مباشرة إلى مجرى الهواء. لا تترك المصاب وحده للاتصال إذا كان هناك شخص آخر متاح.
P) A + B = Airway and Breathing.
   ع) A + B = مجرى الهواء والتنفس.
   A — AIRWAY: Keep the head in the position you found it and look inside the mouth. If you see any solid or liquid obstruction → roll the casualty onto their side and clear it. If nothing is seen → leave them on their back and open the airway using HEAD TILT / CHIN LIFT: one hand on the forehead tilting the head back, two fingertips under the bony point of the chin lifting it up. B — BREATHING: With the airway open, take no longer than 10 SECONDS to: LOOK for chest and upper-abdomen movement, LISTEN for normal breathing, FEEL the breath on your cheek. Normal is 12–24 breaths per minute. Occasional gasping is NOT normal breathing — treat it as cardiac arrest. IF BREATHING NORMALLY → recovery position, call 998, monitor continuously. IF NOT BREATHING NORMALLY → call 998 (if not already done) and start CPR immediately.
   A — مجرى الهواء: أبقِ الرأس بالوضعية التي وجدته عليها وانظر داخل الفم. إذا رأيت أي انسداد صلب أو سائل ← أدر المصاب على جنبه ونظّفه. إذا لم ترَ شيئاً ← اتركه على ظهره وافتح المجرى بإمالة الرأس ورفع الذقن: يد على الجبهة تميل الرأس للخلف، وإصبعان تحت الجزء العظمي من الذقن يرفعانه. B — التنفس: مع فتح المجرى، خذ 10 ثوانٍ كحد أقصى: انظر لحركة الصدر والبطن العلوي، استمع للتنفس الطبيعي، اشعر بالنَّفَس على خدك. الطبيعي 12–24 نفساً في الدقيقة. اللهاث المتقطع ليس تنفساً طبيعياً — تعامل معه كتوقف قلبي. إذا كان يتنفس طبيعياً ← وضعية الإفاقة، اتصل 998، راقب باستمرار. إذا لم يكن يتنفس طبيعياً ← اتصل 998 وابدأ الإنعاش فوراً.
Q) C = Compressions — CPR technique.
   ف) C = الضغطات — أسلوب الإنعاش القلبي الرئوي.
   HAND POSITION: Heel of one hand on the CENTRE of the chest (lower half of the breastbone). Second hand on top, fingers interlocked and pulled up off the ribs. Arms straight, shoulders directly above your hands. COMPRESSIONS: Press down 5–6 cm (about one third of the chest depth) — then allow FULL recoil without lifting your hands off the chest. RATE: 100–120 compressions per minute. CYCLE: 30 compressions : 2 rescue breaths. RESCUE BREATHS: Head tilted, chin lifted, pinch the nose closed, seal your mouth over theirs, blow steadily for 1 second until the chest rises, let it fall, repeat once. Use a pocket mask or face shield from the first aid kit whenever available. IF UNWILLING OR UNABLE TO GIVE BREATHS: Do continuous compressions only — compression-only CPR is far better than doing nothing. STOP CPR ONLY WHEN: Professionals arrive and take over. The casualty starts breathing normally. The area becomes unsafe. You are too exhausted to continue. The casualty begins to vomit (roll to side, clear airway, resume). Another competent first aider relieves you. A doctor pronounces death. SWAP RESCUERS every 2 minutes if possible — compression quality drops fast with fatigue.
   موضع اليدين: كعب يد واحدة في منتصف الصدر (النصف السفلي من عظم القص). اليد الثانية فوقها والأصابع متشابكة ومرفوعة عن الأضلاع. الذراعان مستقيمتان والكتفان فوق اليدين مباشرة. الضغطات: اضغط 5–6 سم (نحو ثلث عمق الصدر) ثم اسمح بالارتداد الكامل دون رفع يديك عن الصدر. المعدل: 100–120 ضغطة في الدقيقة. الدورة: 30 ضغطة : نفَسان إنقاذيان. النفَس الإنقاذي: الرأس مائل والذقن مرفوع، اقفل الأنف، أحكم فمك على فمه، انفخ بثبات لمدة ثانية حتى يرتفع الصدر، دعه ينزل، وكرر مرة. استخدم قناع الجيب أو واقي الوجه من حقيبة الإسعاف متى توفر. إذا لم تستطع أو لم ترغب بإعطاء أنفاس: اكتفِ بالضغطات المتواصلة — الضغطات وحدها أفضل بكثير من لا شيء. توقف عن الإنعاش فقط عند: وصول المختصين وتسلّمهم الحالة. عودة التنفس الطبيعي. أن تصبح المنطقة غير آمنة. الإرهاق التام. بدء المصاب بالتقيؤ (أدره على جنبه، نظّف المجرى، استأنف). وصول مسعف كفؤ آخر ليحل محلك. إعلان الوفاة من طبيب. بدّل المنقذين كل دقيقتين إن أمكن — جودة الضغط تنخفض بسرعة مع التعب.
R) The recovery position — for an unconscious casualty who IS breathing.
   ص) وضعية الإفاقة — لفاقد الوعي الذي يتنفس.
   WHY: An unconscious person on their back can lose their airway to their own tongue, or choke on vomit. The recovery position keeps the airway open and lets fluid drain out. HOW: 1) Kneel beside them. Straighten both legs. 2) Place the arm nearest you out at right angles, elbow bent, palm facing up. 3) Bring the far arm across the chest and hold the back of that hand against the near cheek. 4) With your other hand, pull the far knee up so the foot is flat on the floor. 5) Pull on the raised knee to roll them towards you onto their side. 6) Adjust the upper leg so hip and knee are bent at right angles. 7) Tilt the head back gently to keep the airway open. MONITOR: Check breathing continuously until the ambulance arrives. If they stop breathing normally → roll onto back and start CPR. SUSPECTED SPINAL INJURY: Still prioritise the airway — an open airway beats a protected spine, but move them with the head and body kept in line.
   لماذا: فاقد الوعي على ظهره قد يفقد مجرى هواءه بسبب لسانه أو يختنق بالقيء. وضعية الإفاقة تُبقي المجرى مفتوحاً وتسمح للسوائل بالخروج. الطريقة: 1) اركع بجانبه وافرد ساقيه. 2) ضع الذراع الأقرب إليك بزاوية قائمة والمرفق مثني والكف لأعلى. 3) اسحب الذراع البعيدة عبر الصدر وضع ظهر كفها على الخد القريب. 4) بيدك الأخرى ارفع الركبة البعيدة حتى تستقر القدم على الأرض. 5) اسحب الركبة المرفوعة لتدويره نحوك على جنبه. 6) اضبط الساق العلوية بحيث يكون الورك والركبة بزاوية قائمة. 7) أمِل الرأس للخلف برفق لإبقاء المجرى مفتوحاً. المراقبة: افحص التنفس باستمرار حتى وصول الإسعاف. إذا توقف التنفس الطبيعي ← أعده على ظهره وابدأ الإنعاش. عند الاشتباه بإصابة العمود الفقري: الأولوية تبقى لمجرى الهواء، لكن حرّكه مع إبقاء الرأس والجسم على استقامة واحدة.
S) Choking — partial and complete airway obstruction.
   ق) الاختناق — انسداد جزئي أو كامل لمجرى الهواء.
   RECOGNISE: PARTIAL — casualty is coughing forcefully, can speak or wheeze. ACTION: ENCOURAGE THEM TO KEEP COUGHING. Do not slap the back of someone who is coughing effectively. Stay with them. COMPLETE — cannot speak, cough, or breathe; may clutch the throat; face turns red then blue; silent. ACT IMMEDIATELY. BACK BLOWS: Stand to the side and slightly behind. Support the chest with one hand and lean the upper body well forward. Give up to 5 SHARP blows between the shoulder blades with the heel of your hand. Check after each one — stop as soon as the obstruction clears. ABDOMINAL THRUSTS: If back blows fail, stand behind, place one foot between theirs for balance, put both arms around the upper abdomen. Make a fist and place it between the navel and the rib cage, grasp it with the other hand, and pull sharply INWARD AND UPWARD. Up to 5 thrusts. ALTERNATE 5 back blows and 5 thrusts until cleared. IF THEY BECOME UNRESPONSIVE: Lower them to the floor, call 998, and start CPR immediately — compressions can dislodge the obstruction. AFTER ANY ABDOMINAL THRUST: The casualty MUST be seen by a doctor — thrusts can cause internal injury.
   التمييز: انسداد جزئي — يسعل بقوة ويستطيع الكلام أو الصفير. الإجراء: شجّعه على مواصلة السعال. لا تضرب ظهر من يسعل بفعالية. ابقَ معه. انسداد كامل — لا يستطيع الكلام أو السعال أو التنفس؛ قد يمسك حلقه؛ يحمرّ وجهه ثم يزرقّ؛ صامت. تصرّف فوراً. الضربات الظهرية: قف إلى الجانب وخلفه قليلاً. اسند الصدر بيد وأمِل الجذع العلوي للأمام جيداً. وجّه حتى 5 ضربات حادة بين لوحي الكتف بكعب اليد. تفقّد بعد كل ضربة وتوقف فور زوال الانسداد. الدفعات البطنية: إذا فشلت الضربات، قف خلفه وضع قدماً بين قدميه للتوازن ولفّ ذراعيك حول أعلى البطن. اقبض يدك وضعها بين السرة والقفص الصدري، أمسكها باليد الأخرى، واسحب بقوة للداخل وللأعلى. حتى 5 دفعات. بادِل بين 5 ضربات ظهرية و5 دفعات حتى يزول الانسداد. إذا فقد الوعي: أنزله على الأرض، اتصل 998، وابدأ الإنعاش فوراً — الضغطات قد تُخرج الانسداد. بعد أي دفعة بطنية: يجب عرض المصاب على طبيب — الدفعات قد تسبب إصابة داخلية.
T) Shock — recognition and management.
   ر) الصدمة — التعرف والتعامل.
   WHAT IT IS: A life-threatening loss of effective circulation — not enough blood/oxygen reaching the organs. CAUSES ON SITE: Severe bleeding, major burns, crush injury, spinal injury, heart attack, extreme dehydration, severe allergic reaction. SIGNS: Pale, cold, clammy, sweaty skin. Rapid weak pulse. Fast shallow breathing. Nausea, thirst, dizziness. Anxiety, confusion, then drowsiness and unconsciousness. MANAGEMENT: 1) Treat the CAUSE first — stop the bleeding, cool the burn. 2) Lay the casualty down. 3) Raise and support the legs about 20–30 cm (8–10 inches) — unless a fractured leg or spinal injury is suspected. 4) Loosen tight clothing at the neck, chest, and waist. 5) Keep them warm — cover with a blanket or coat, but do not overheat. 6) DO NOT give food, drink, or cigarettes — if they are thirsty, moisten the lips only. 7) Reassure them and keep them still. 8) Call 998. 9) Monitor breathing and response continuously — if they stop breathing normally, start CPR.
   ما هي: فقدان مهدد للحياة للدورة الدموية الفعّالة — عدم وصول دم/أكسجين كافٍ للأعضاء. أسبابها في الموقع: نزيف شديد، حروق كبيرة، إصابة سحق، إصابة عمود فقري، نوبة قلبية، جفاف شديد، حساسية شديدة. العلامات: جلد شاحب بارد رطب متعرق. نبض سريع ضعيف. تنفس سريع سطحي. غثيان وعطش ودوخة. قلق وارتباك ثم نعاس وفقدان وعي. التعامل: 1) عالج السبب أولاً — أوقف النزيف، برّد الحرق. 2) أضجع المصاب. 3) ارفع الساقين واسندهما نحو 20–30 سم — إلا عند الاشتباه بكسر في الساق أو إصابة عمود فقري. 4) فكّ الملابس الضيقة عند الرقبة والصدر والخصر. 5) دفّئه بغطاء دون إفراط. 6) لا تعطه طعاماً أو شراباً أو سجائر — إذا عطش بلّل شفتيه فقط. 7) طمئنه وأبقِه ساكناً. 8) اتصل 998. 9) راقب التنفس والاستجابة باستمرار — إذا توقف التنفس الطبيعي ابدأ الإنعاش.
U) Amputation and crush injury.
   ش) البتر وإصابات السحق.
   AMPUTATION — when a body part is severed or torn off. MANAGEMENT: 1) Follow DRABC first. 2) Control the bleeding — firm direct pressure on the stump with a sterile dressing; add layers on top, do not remove soaked dressings. 3) Elevate the injured limb above heart level. 4) Treat for shock. 5) Call 998 IMMEDIATELY. THE SEVERED PART — this is what saves the limb: Do NOT wash it. Wrap it in sterile gauze or clean cloth, place it in a sealed plastic bag or airtight container. Place THAT bag into a second container of ice and water. THE SEVERED PART MUST NEVER TOUCH ICE OR WATER DIRECTLY — direct contact causes frostbite damage and makes reattachment impossible. Label with the casualty's name and the time of injury. Send it to hospital WITH the casualty. CRUSH INJURY: If a limb has been trapped under load for a long period, tell the paramedics exactly how long — releasing a long-crushed limb can be fatal, and that decision belongs to the medical team, not to you.
   البتر — عندما يُقطع أو يُنتزع جزء من الجسم. التعامل: 1) اتبع DRABC أولاً. 2) أوقف النزيف — ضغط مباشر قوي على الجذمور بضمادة معقمة؛ أضف طبقات فوقها ولا تنزع الضمادة المشبعة. 3) ارفع الطرف المصاب فوق مستوى القلب. 4) عالج الصدمة. 5) اتصل 998 فوراً. الجزء المبتور — هذا ما ينقذ الطرف: لا تغسله. لُفّه بشاش معقم أو قماش نظيف وضعه في كيس بلاستيكي محكم أو وعاء مغلق. ضع ذلك الكيس داخل وعاء ثانٍ فيه ثلج وماء. يجب ألا يلامس الجزء المبتور الثلج أو الماء مباشرة أبداً — الملامسة المباشرة تسبب ضرر تجمّد وتجعل إعادة الوصل مستحيلة. اكتب عليه اسم المصاب ووقت الإصابة. أرسله للمستشفى مع المصاب. إصابة السحق: إذا حُبس طرف تحت حمل لفترة طويلة، أخبر المسعفين بالمدة بدقة — تحرير طرف مسحوق لفترة طويلة قد يكون قاتلاً، والقرار للفريق الطبي لا لك.
V) First aid kit — inspection and restocking.
   ت) حقيبة الإسعاف — الفحص وإعادة التزويد.
   PLACEMENT: Kits must be distributed so that no worker is far from one — the practical rule is a kit within about 90 m (300 ft) of every work position, plus one in each vehicle. Each kit clearly signed (white cross on green), unlocked, and never blocked. MINIMUM CONTENTS: Blunt-ended scissors. Sterile gauze pads (10×10 cm / 4×4 in). Box of adhesive bandages — BLUE detectable in food areas. Roller bandage (min 5 cm / 2 in). Two triangular bandages. Two elastic wraps. Sealed cleansing wipes. Adhesive tape. Tweezers. Disposable gloves. Eye wash. Burn gel. Emergency blanket. Resuscitation pocket mask / face shield. Splint. Card with emergency numbers and the names of the site first aiders. MONTHLY CHECK: A named person checks every kit monthly — contents complete, nothing expired, seals intact, restocked after every use. Record the check. RULE: A kit that is empty or expired when you need it is the same as no kit at all.
   التوزيع: توزَّع الحقائب بحيث لا يبعد أي عامل عنها — القاعدة العملية حقيبة ضمن نحو 90 متراً من كل موقع عمل، إضافة لواحدة في كل مركبة. كل حقيبة معلّمة بوضوح (صليب أبيض على أخضر) وغير مقفلة ولا يعيقها شيء. الحد الأدنى للمحتويات: مقص بأطراف غير حادة. شاش معقم 10×10 سم. علبة لاصقات جروح — زرقاء قابلة للكشف في مناطق الغذاء. رباط لفّاف 5 سم على الأقل. رباطان مثلثان. رباطان مطاطيان. مناديل تنظيف مغلّفة. شريط لاصق. ملقط. قفازات للاستعمال مرة واحدة. غسول عيون. هلام حروق. بطانية طوارئ. قناع إنعاش/واقي وجه. جبيرة. بطاقة بأرقام الطوارئ وأسماء مسعفي الموقع. الفحص الشهري: شخص محدد يفحص كل حقيبة شهرياً — المحتويات كاملة، لا شيء منتهي الصلاحية، الأختام سليمة، وإعادة تزويد بعد كل استخدام. سجّل الفحص. القاعدة: الحقيبة الفارغة أو المنتهية وقت الحاجة = لا حقيبة.
   References: UAE OHS Federal Law No. 8/1980 | Dubai Municipality H&S Code | ISO 45001:2018 | UAE MOHRE Regulations | BS 8599 (First Aid Kits) | OSHA 1910.151 & 1910.266 App A | SOP-EM-04 (First Aid & Major Injury Response).`,
