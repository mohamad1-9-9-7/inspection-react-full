...
const LOOKUP_VEHICLES_TYPE = "sweets_cars_loading_lookup_vehicle_numbers";
const LOOKUP_DRIVERS_TYPE = "sweets_cars_loading_lookup_driver_names";
...
const TEMP_LIMIT_C = 5;
const tempTooHigh = (v) => String(v ?? "").trim() !== "" && Number(v) > TEMP_LIMIT_C;
const TEMP_HOT = { borderColor: "#dc2626", background: "#fef2f2", color: "#b91c1c", fontWeight: 900 };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await saveToServer({
        ...
        createdAt: Date.now(),
        header,
        // Several loading sheets a day are normal: unique key, plain day in `date`.
        date: reportDate,
        reportDate: eventReportDate(reportDate),
        inspectedBy,
        verifiedBy,
        rows: validCleanRows,
      });
