...
export default function TrainingCertVerify() {
  ...
  // Public page — no session, so the issuing company travels in the QR link.
  const company = params.get('co')     || '';
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ background:'#fff', borderRadius:12, padding:5 }}>
              <CompanyMark size={44} name={company} />
            </div>
            <div>
              <div style={{ color:'rgba(255,255,255,.6)', fontSize:9, letterSpacing:2, textTransform:'uppercase' }}>{company}</div>
          ...
          <div style={{ display:'flex', gap:6, alignItems:'center' }}>
            ...
            >🌐 {isAr ? 'EN' : 'ع'}</button>
            <div style={{ color:'rgba(255,255,255,.3)', fontSize:10, letterSpacing:1 }}>VERIFY</div>
          </div>
        </div>
        ...
        <div style={{ padding:'20px 28px 28px' }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize:9, color:'#cbd5e1' }}>{company ? `${company} — Quality Department` : 'Quality Department'}</span>
