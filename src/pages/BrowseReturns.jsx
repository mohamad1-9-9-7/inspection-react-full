  ...
  const entries = raw
    .map((rec) => {
      ...
      return {
        ...
        refNo,
        /* The two names the input sheet was closed with. They belong on the
           PDF and in the e-mail: a report that reaches a manager without
           saying who filled it in and who checked it is not evidence. */
...
export default function BrowseReturns() {
      ...
      <div style="font-family:'Segoe UI',Inter,Roboto,Arial,sans-serif;background:#eef2f7;padding:24px 16px;color:#0f172a;">
        <div style="max-width:980px;margin:auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;">
          ...
          <div style="padding:22px 28px;">
            ...
            <!-- Who filled the sheet in, and who checked it. A report that
                 arrives in a manager's inbox unsigned is not evidence, so the
                 two lines print even when a name is missing. -->
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:18px;border-collapse:collapse;">
              <tr>
                <td width="50%" style="padding:12px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;">
                  <div style="font-size:10px;font-weight:800;letter-spacing:.6px;color:#64748b;text-transform:uppercase;">Checked by</div>
                  <div style="margin-top:4px;font-size:14px;font-weight:800;color:${rep?.checkedBy ? "#0f172a" : "#94a3b8"};">${escapeHtml(rep?.checkedBy || "not signed")}</div>
                </td>
                <td width="8"></td>
                <td width="50%" style="padding:12px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;">
                  <div style="font-size:10px;font-weight:800;letter-spacing:.6px;color:#64748b;text-transform:uppercase;">Verified by</div>
                  <div style="margin-top:4px;font-size:14px;font-weight:800;color:${rep?.verifiedBy ? "#0f172a" : "#94a3b8"};">${escapeHtml(rep?.verifiedBy || "not signed")}</div>
                </td>
              </tr>
            </table>
          </div>
  ...
  const buildReturnsText = useCallback((rep, opts = {}) => {
    ...
    // the same two signatures the HTML mail and the PDF close with
    lines.push(`Checked by: ${rep?.checkedBy || "not signed"}`);
    lines.push(`Verified by: ${rep?.verifiedBy || "not signed"}`);
    lines.push("");
  ...
  const handleExportPDF = async (opts = {}) => {
    ...
    try {
      ...
      /* ── Signatures ──
         The sheet closes the way the paper one does: who filled it in, who
         checked it. A report with no names still prints the two lines, so the
         gap is visible rather than silently absent. If the table ended near
         the foot of the page, the block starts a fresh one instead of being
         squeezed into the margin. */
      {
        const pageH = doc.internal.pageSize.getHeight();
        const sigH = 46;
        let y = (doc.lastAutoTable?.finalY || marginTop) + 22;
        if (y + sigH > pageH - 40) {
          doc.addPage();
          drawHeader();
          drawWatermark();
        ...
        pairs.forEach(([label, name], i) => {
          ...
          doc.setFont("helvetica", name ? "bold" : "italic");
          doc.setFontSize(11);
          doc.setTextColor(name ? 15 : 148, name ? 23 : 163, name ? 42 : 184);
          doc.text(String(name || "not signed"), x, y + 16);

          doc.setDrawColor(203, 213, 225);
          doc.setLineWidth(0.7);
          doc.line(x, y + 24, x + colW - 24, y + 24);
        });
                  ...
                  <>
                    ...
                    <div className="br-noprint" style={{
                      ...
                      <div>
                        ...
                        {/* Who signed this day's sheet. It sits here, next to
                            the e-mail button, so the sender sees what the
                            recipient is about to read at the foot of it. */}
                        <div style={{ display: "flex", gap: 14, marginTop: 6, flexWrap: "wrap", fontSize: 12, color: T.textM }}>
                          <span>
                            ✍️ Checked by{" "}
                            <b style={{ color: selectedReport.checkedBy ? T.text : T.textM, fontStyle: selectedReport.checkedBy ? "normal" : "italic" }}>
                              {selectedReport.checkedBy || "not signed"}
                            </b>
                          </span>
                          <span>
                            🔎 Verified by{" "}
                            <b style={{ color: selectedReport.verifiedBy ? T.text : T.textM, fontStyle: selectedReport.verifiedBy ? "normal" : "italic" }}>
