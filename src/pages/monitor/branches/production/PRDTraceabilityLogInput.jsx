...
export default function PRDTraceabilityLogInput() {
  ...
  async function handleSave() {
    ...
    if (!hasAnyData) {
      alert("الرجاء تعبئة دفعة واحدة على الأقل قبل الحفظ.");
      return;
    }
    ...
    // الاعتماد على حقول الدلالات لتحديد امتلاء السطر
    ...
    try {
      const res = await fetch(`${API_BASE}/api/reports`, {
        ...
        body: JSON.stringify({ reporter: "production", type: TYPE, payload }), // ⬅️ للإنتاج
      });
    ...
    <div style={{ background:"#fff", border:"1px solid #dbe3f4", borderRadius:12, padding:16, color:"#0b1f4d" }}>
      ...
      >
        ...
        <span style={{ marginInlineStart: 4 }}>
          Single-row per <b>Batch / Lot</b> with dynamic RAW/FINAL lines. Data is flattened to legacy
          <b> entries</b> array on save for backend compatibility.
        </span>
      </div>
    </div>
  );
