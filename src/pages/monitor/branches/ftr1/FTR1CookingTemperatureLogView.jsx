// src/pages/monitor/branches/ftr1/FTR1CookingTemperatureLogView.jsx
...
/* Utilities to keep payload clean/consistent */
const clean = (obj) =>
  Object.fromEntries(
...
export default function FTR1CookingTemperatureLogView() {
  ...
  const handleExportPDF = async () => {
    ...
    pdf.save(`FTR1_CookingTemp_${selectedReport?.payload?.reportDate || "report"}.pdf`);
  ...
  const handleExportXLS = () => {
    ...
    a.href = url; a.download = `FTR1_CookingTemp_${cur.reportDate || "report"}.xls`;
  ...
  const handleExportJSON = () => {
    try {
      ...
      a.href = url; a.download = `FTR1_CookingTemp_ALL_${ts}.json`;
      document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  ...
  const handleDelete = async (report) => {
    ...
    try {
      ...
      alert("✅ Report deleted successfully."); fetchReports();
  ...
  const saveEdit = async () => {
    ...
    // بإمكانك طلب كلمة سر هنا أيضًا إن رغبت:
    ...
    try {
      ...
      // اختيارياً: محاولة معرفة الـID الجديد
      let newId = null;
      try {
      ...
      setEditMode(false);
      setDraft(null);
      await fetchReports(); // سنجلب الأحدث بعد الحفظ
      alert("✅ Changes saved (deleted old and uploaded new).");
    } catch (e) {
      console.error(e);
