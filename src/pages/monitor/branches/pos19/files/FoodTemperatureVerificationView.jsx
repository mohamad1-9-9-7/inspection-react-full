// src/pages/monitor/branches/pos19/pos19_views/FoodTemperatureVerificationView.jsx
...
function emptyRow(){ return { date:"", time:"", foodItem:"", processType:"cooking", targetTemp:"≥ 75°C", actualTemp:"", result:"", correctiveAction:"", checkedBy:"" }; }
...
export default function FoodTemperatureVerificationView() {
  ...
  const colDefs = useMemo(()=>[
    <col key="date" style={{width:110}}/>,<col key="time" style={{width:90}}/>,<col key="food" style={{width:180}}/>,
    <col key="proc" style={{width:120}}/>,<col key="target" style={{width:120}}/>,<col key="actual" style={{width:110}}/>,
    <col key="result" style={{width:90}}/>,<col key="ca" style={{width:200}}/>,<col key="chk" style={{width:130}}/>,
  ],[]);
  ...
  function updateRow(i,key,val){setEditRows(p=>{const n=[...p];n[i]={...n[i],[key]:val};return n;});}
  ...
  function exportJSON(){if(!record)return;const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify({type:TYPE,payload:record.payload},null,2)],{type:"application/json"}));a.download=`POS19_FoodTempVerification_${record?.payload?.reportDate||date}.json`;a.click();URL.revokeObjectURL(a.href);}

  async function exportXLSX(){
    try{
      const ExcelJS=(await import("exceljs")).default||(await import("exceljs"));
      const p=record?.payload||{};const rows=(p.entries||[]).filter(isFilledRow);
      const wb=new ExcelJS.Workbook();const ws=wb.addWorksheet("FoodTempVerification");
      ...
      const COL_HEADERS=["Date","Time","Food Item","Process Type","Target Temp","Actual Temp (°C)","Result (Pass/Fail)","Corrective Action","Checked by"];
      ws.columns=[{width:12},{width:10},{width:24},{width:14},{width:16},{width:14},{width:14},{width:28},{width:16}];
      ws.mergeCells(1,1,1,COL_HEADERS.length);const r1=ws.getCell(1,1);r1.value=`POS 19 | Food Temperature Verification Log — ${FORM_REF}`;r1.alignment={horizontal:"center",vertical:"middle"};r1.font={size:13,bold:true};r1.fill={type:"pattern",pattern:"solid",fgColor:{argb:"E9F0FF"}};ws.getRow(1).height=22;
      ws.mergeCells(2,1,2,COL_HEADERS.length);ws.getCell(2,1).value=`Branch: ${BRANCH} | Date: ${p.reportDate||""} | Critical Limits: Cooking/Reheating ≥75°C | Chilled ≤5°C | Frozen ≤-18°C`;ws.getCell(2,1).alignment={horizontal:"center"};ws.getRow(2).height=18;
      ...
      let rIdx=5;rows.forEach(e=>{
        ws.getRow(rIdx).values=[e?.date||"",e?.time||"",e?.foodItem||"",e?.processType||"",e?.targetTemp||"",e?.actualTemp||"",e?.result||"",e?.correctiveAction||"",e?.checkedBy||""];
        ws.getRow(rIdx).eachCell((cell,col)=>{cell.alignment={horizontal:"center",vertical:"middle",wrapText:true};cell.border=border;if(col===7&&e?.result==="Pass")cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:"E7F7EC"}};if(col===7&&e?.result==="Fail")cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FDE8E8"}};});ws.getRow(rIdx).height=20;rIdx++;
      });
      ...
      const buf=await wb.xlsx.writeBuffer({useStyles:true,useSharedStrings:true});const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([buf],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}));a.download=`POS19_FoodTempVerification_${p.reportDate||date}.xlsx`;a.click();URL.revokeObjectURL(a.href);
  ...
  async function exportPDF(){
    ...
    const drawH=()=>{pdf.setFillColor(233,240,255);pdf.rect(0,0,pageW,headerH,"F");pdf.setFont("helvetica","bold");pdf.setFontSize(13);pdf.text(`POS 19 | Food Temperature Verification Log — ${record?.payload?.reportDate||date}`,pageW/2,28,{align:"center"});};
    ...
    pdf.save(`POS19_FoodTempVerification_${record?.payload?.reportDate||date}.pdf`);
  }
  ...
  return (
    <div style={{background:"#fff",border:"1px solid #dbe3f4",borderRadius:12,padding:16,color:"#0b1f4d",direction:"ltr"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
        <div style={{fontWeight:800,fontSize:18}}>Food Temperature Verification Log — View (POS 19)</div>
        <div style={{marginInlineStart:"auto",display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          {editing&&<><button onClick={addRow} style={btn("#16a34a")}>+ Add Row</button><button onClick={saveEdit} style={btn("#10b981")}>Save Changes</button></>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:12}}>
        ...
        <div>
          ...
          {record&&(<div ref={reportRef}>
            ...
            <div style={{border:"1px solid #1f3b70",borderBottom:"none"}}><div style={{...thCell,background:"#e9f0ff"}}>Critical Limits: Cooking/Reheating ≥ 75°C | Chilled ≤ 5°C | Frozen ≤ -18°C | Cooling ≤ 5°C within 4 hrs</div></div>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed",fontSize:12}}>
                <colgroup>{colDefs}</colgroup>
                <thead><tr>
                  <th style={thCell}>Date</th><th style={thCell}>Time</th><th style={thCell}>Food Item</th>
                  <th style={thCell}>Process{"\n"}Type</th><th style={thCell}>Target{"\n"}Temp</th><th style={thCell}>Actual{"\n"}Temp (°C)</th>
                  <th style={thCell}>Result{"\n"}(Pass/Fail)</th><th style={thCell}>Corrective{"\n"}Action</th><th style={thCell}>Checked{"\n"}by</th>
                </tr></thead>
                <tbody>
                  {!editing?(rows.filter(isFilledRow).map((r,idx)=>(<tr key={idx}>
                    <td style={tdCell}>{formatDMY(safe(r.date))}</td><td style={tdCell}>{safe(r.time)}</td><td style={tdCell}>{safe(r.foodItem)}</td>
                    <td style={tdCell}>{safe(r.processType)}</td><td style={tdCell}>{safe(r.targetTemp)}</td>
                    <td style={{...tdCell,background:r.result==="Fail"?"#fde8e8":""}}>{safe(r.actualTemp)}</td>
                    ...
                    editRows.map((r,i)=>(<tr key={i}>
                      ...
                      <td style={tdCell}><input type="text" value={r.foodItem||""} onChange={e=>updateRow(i,"foodItem",e.target.value)} style={inputStyle}/></td>
                      <td style={tdCell}><select value={r.processType||""} onChange={e=>{const found=PROCESS_TYPES.find(p=>p.value===e.target.value);updateRow(i,"processType",e.target.value);if(found)updateRow(i,"targetTemp",found.limit);}} style={inputStyle}>{PROCESS_TYPES.map(p=><option key={p.value} value={p.value}>{p.label}</option>)}</select></td>
                      <td style={tdCell}><input type="text" value={r.targetTemp||""} onChange={e=>updateRow(i,"targetTemp",e.target.value)} style={inputStyle}/></td>
                      <td style={{...tdCell,background:r.result==="Fail"?"#fde8e8":""}}><input type="number" step="0.1" value={r.actualTemp||""} onChange={e=>updateRow(i,"actualTemp",e.target.value)} style={{...inputStyle,background:r.result==="Fail"?"#fde8e8":"#fff"}}/></td>
                      <td style={{...tdCell,background:r.result==="Pass"?"#e7f7ec":r.result==="Fail"?"#fde8e8":""}}>
                        <select value={r.result||""} onChange={e=>updateRow(i,"result",e.target.value)} style={inputStyle}><option value=""></option><option value="Pass">Pass</option><option value="Fail">Fail</option></select>
                      </td>
                      <td style={tdCell}><input type="text" value={r.correctiveAction||""} onChange={e=>updateRow(i,"correctiveAction",e.target.value)} style={{...inputStyle,background:r.result==="Fail"?"#fff7ed":"#fff"}} placeholder={r.result==="Fail"?"Required":""}/></td>
