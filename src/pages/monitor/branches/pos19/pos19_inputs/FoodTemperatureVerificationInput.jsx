  ...
  return (
    <div style={{ background:C.gray50, fontFamily:"'Segoe UI',system-ui,sans-serif", color:C.gray700, direction:"ltr", borderRadius:12, overflow:"hidden" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, padding:"14px 16px 0" }}>
        {[
          ...
          <div key={label} style={{ background:C.accentBg, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 14px" }}>
            <div style={{ fontSize:11, color:C.accent, fontWeight:700, letterSpacing:.5, textTransform:"uppercase", marginBottom:4 }}>{label}</div>
            {val !== null
              ? <div style={{ fontSize:15, fontWeight:700, color:C.navy }}>{val}</div>
              : <input type="date" value={reportDate} onChange={e=>setReportDate(e.target.value)}
                  style={{ ...inputStyle, border:"none", background:"transparent", fontWeight:700, fontSize:15, color:C.navy, padding:0 }} />
            }
          </div>
        ))}
      </div>
      ...
      <div style={{ padding:"10px 16px 0", display:"flex", alignItems:"center", gap:10 }}>
        <div style={{ fontSize:12, color:C.navy, fontWeight:700 }}>Section:</div>
        <input value={section} onChange={e=>setSection(e.target.value)}
          style={{ ...inputStyle, maxWidth:240, borderColor:C.border }}
