    ...
    <div style={{ background:"#fff", border:"1px solid #dbe3f4", borderRadius:12, padding:16, color:"#0b1f4d" }}>
          ...
          <tbody>
            {rows.map((r, idx) => (
              <tr key={idx}>
                ...
                <td style={tdCell}><input type="text" value={r.invoiceNo}        onChange={(e)=>updateRow(idx,"invoiceNo",e.target.value)}        style={inputStyle} /></td>
      ...
      <div style={{ marginTop:10, fontSize:11, color:"#0b1f4d" }}>
        ...
        <div style={{ marginTop:8 }}>
          <strong>Note:</strong> For Chilled Food: Target ≤ 5°C (Critical Limit: 5°C; short deviations up to 15 minutes during transfer). For Frozen Food: Target ≤ -18°C (Critical limits: RTE Frozen ≤ -18°C, Raw Frozen ≤ -10°C). For Hot Food: Target ≥ 60°C (Critical Limit: 60°C). Dry food, Low Risk: Receive at cool, dry condition or ≤ 25°C, or as per product requirement.
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginTop:12, flexWrap:"wrap" }}>
        <button onClick={handleSave} disabled={saving || dateStatus.blocked} style={btn("#2563eb")}>
          {saving ? "Saving…" : dateStatus.blocked ? "🔒 محفوظ مسبقاً" : "Save Receiving Log"}
        </button>
      </div>
