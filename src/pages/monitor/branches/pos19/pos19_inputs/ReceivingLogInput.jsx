...
export default function ReceivingLogInput() {
  // تاريخ التقرير الهيدر (غير عن تاريخ كل صف)
  ...
  // عدد الصفوف (غيره لو بدك)
  const ROW_COUNT = 15;
  const [rows, setRows] = useState(() => Array.from({ length: ROW_COUNT }, () => emptyRow()));

  // هيدر
  const [formRef, setFormRef] = useState("UC/HACCP/BR/F01A");
  ...
  // month text لو حاب تحفظه مع الحمولة
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      formRef,
      classification,
      reportDate,         // تاريخ التقرير (الهيدر)
      month: monthText,
      entries,            // الصفوف
      verifiedBy,
      revDate,
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #dbe3f4", borderRadius:12, padding:16, color:"#0b1f4d" }}>
      ...
      {/* Footer line: Verified by + Rev fields */}
