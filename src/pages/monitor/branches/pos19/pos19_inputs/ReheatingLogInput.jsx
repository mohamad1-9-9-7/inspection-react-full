...
const EMPTY_ROW = () => ({
  foodItem: "", batchNo: "", method: "",
  startTime: "", initialTemp: "",
  endTime: "", finalTemp: "",
  destination: "", correctiveAction: "",
});

const METHODS = ["Oven", "Stovetop", "Steamer", "Microwave", "Bain-marie", "Other"];

const HELP = {
  foodItem:        "Name of the cooked food being reheated.",
  batchNo:         "Batch / Lot number for traceability.",
  method:          "Reheating method used (Oven, Stovetop, Steamer, Microwave, Bain-marie).",
  startTime:       "Time reheating started (HH:MM, 24h).",
  initialTemp:     "Internal temperature of the food before reheating.",
  endTime:         "Time when reheating reached its target temperature.",
  finalTemp:       "Final internal temperature. CCP limit: must be ≥ 75°C, held for at least 15 seconds.",
  destination:     "Where the food goes after reheating (e.g., served immediately, hot holding unit).",
  correctiveAction:"Action taken if reheating limits were not met (e.g., reheat again, discard).",
};

export default function ReheatingLogInput() {
  ...
  const isFinalLow = (v) => v && parseFloat(v) < 75;

  return (
    <div style={S.wrap}>
      <ReportHeader
        title="Reheating Temperature Log"
        titleAr="سجل إعادة تسخين الطعام"
      ...
      <div style={S.banner}>
        ...
        <div>
          <strong>Critical Limit (CCP):</strong> Reheated food must reach an internal temperature of
          <b> ≥ 75°C</b> for at least <b>15 seconds</b>. Reheating must be done in a <b>single, rapid pass</b>
          (target ≤ 2 hours). Food is reheated <b>once only</b>; reject for any second reheating.
        </div>
      </div>
      ...
      <div style={S.tableWrap}>
        <table style={S.table}>
          <colgroup>
            ...
            <col style={{width:100}}/><col style={{width:100}}/>
            <col style={{width:140}}/><col style={{width:170}}/>
            <col style={{width:50}}/>
          </colgroup>
          <thead>
            <tr>
              <th style={S.th}>#</th>
              <th style={S.th} title={HELP.foodItem}>Food Item ⓘ</th>
              <th style={S.th} title={HELP.batchNo}>Batch No ⓘ</th>
              <th style={S.th} title={HELP.method}>Method ⓘ</th>
              <th style={{...S.th, background:"#fef3c7"}} title={HELP.startTime}>Start Time ⓘ</th>
              <th style={{...S.th, background:"#fef3c7"}} title={HELP.initialTemp}>Initial °C ⓘ</th>
              <th style={{...S.th, background:"#dcfce7"}} title={HELP.endTime}>End Time ⓘ</th>
              <th style={{...S.th, background:"#dcfce7"}} title={HELP.finalTemp}>Final °C ⓘ</th>
              <th style={S.th} title={HELP.destination}>Destination ⓘ</th>
          ...
          <tbody>
            {entries.map((r, i) => (
              <tr key={i}>
                ...
                <td style={S.td}><input value={r.foodItem} onChange={e=>chRow(i,"foodItem",e.target.value)} style={S.input} title={HELP.foodItem} placeholder="e.g., Beef Stew"/></td>
                <td style={S.td}><input value={r.batchNo}  onChange={e=>chRow(i,"batchNo",e.target.value)}  style={S.input} title={HELP.batchNo}/></td>
                <td style={S.td}>
                  <select value={r.method} onChange={e=>chRow(i,"method",e.target.value)} style={S.input} title={HELP.method}>
                    <option value="">—</option>
                    {METHODS.map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                </td>
                <td style={S.td}><input type="time" value={r.startTime} onChange={e=>chRow(i,"startTime",e.target.value)} style={S.input} title={HELP.startTime}/></td>
                <td style={S.td}><input type="number" step="0.1" value={r.initialTemp} onChange={e=>chRow(i,"initialTemp",e.target.value)} style={S.input} title={HELP.initialTemp} placeholder="°C"/></td>
                <td style={S.td}><input type="time" value={r.endTime} onChange={e=>chRow(i,"endTime",e.target.value)} style={S.input} title={HELP.endTime}/></td>
                <td style={S.td}><input type="number" step="0.1" value={r.finalTemp} onChange={e=>chRow(i,"finalTemp",e.target.value)} style={{...S.input, ...(isFinalLow(r.finalTemp)?S.warn:{})}} title={HELP.finalTemp} placeholder="≥75°C"/></td>
                <td style={S.td}><input value={r.destination} onChange={e=>chRow(i,"destination",e.target.value)} style={S.input} title={HELP.destination} placeholder="Served / Hot hold"/></td>
