...
function emptyRow(){ return {date:"",time:"",sanitizerType:"",location:"",targetConc:"200 ppm",actualConc:"",result:"",correctiveAction:"",checkedBy:""}; }
...
export default function SanitizerConcentrationVerificationView() {
  ...
  function exportJSON(){if(!record)return;const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify({type:TYPE,payload:record.payload},null,2)],{type:"application/json"}));a.download=`POS19_SanitizerConc_${record?.payload?.reportDate||date}.json`;a.click();URL.revokeObjectURL(a.href);}
  ...
  async function exportXLSX(){
    try{
      ...
      const COL_HEADERS=["Date","Time","Sanitizer Type","Location / Area","Target Conc. (ppm)","Actual Conc. (ppm)","Result (Pass/Fail)","Corrective Action","Checked by"];
      ws.columns=[{width:12},{width:10},{width:18},{width:20},{width:16},{width:16},{width:14},{width:28},{width:16}];
      ws.mergeCells(1,1,1,COL_HEADERS.length);const r1=ws.getCell(1,1);r1.value=`POS 19 | Sanitizer Concentration Verification Log — ${FORM_REF}`;r1.alignment={horizontal:"center",vertical:"middle"};r1.font={size:13,bold:true};r1.fill={type:"pattern",pattern:"solid",fgColor:{argb:"E9F0FF"}};ws.getRow(1).height=22;
      ws.mergeCells(2,1,2,COL_HEADERS.length);ws.getCell(2,1).value=`Branch: ${BRANCH} | Date: ${safe(p.reportDate)} | Target: 200 ppm | Acceptable Range: 100–400 ppm`;ws.getCell(2,1).alignment={horizontal:"center"};ws.getRow(2).height=18;
      ...
      let rIdx=5;rows.forEach(e=>{
        ws.getRow(rIdx).values=[safe(e.date),safe(e.time),safe(e.sanitizerType),safe(e.location),safe(e.targetConc),safe(e.actualConc),safe(e.result),safe(e.correctiveAction),safe(e.checkedBy)];
      ...
      const buf=await wb.xlsx.writeBuffer({useStyles:true,useSharedStrings:true});const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([buf],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}));a.download=`POS19_SanitizerConc_${p.reportDate||date}.xlsx`;a.click();URL.revokeObjectURL(a.href);
  ...
  async function exportPDF(){
    ...
    const drawH=()=>{pdf.setFillColor(233,240,255);pdf.rect(0,0,pageW,headerH,"F");pdf.setFont("helvetica","bold");pdf.setFontSize(13);pdf.text(`POS 19 | Sanitizer Concentration Verification Log — ${record?.payload?.reportDate||date}`,pageW/2,28,{align:"center"});};
    ...
    pdf.save(`POS19_SanitizerConc_${record?.payload?.reportDate||date}.pdf`);
  }
  ...
  return (
    <div style={{background:"#fff",border:"1px solid #dbe3f4",borderRadius:12,padding:16,color:"#0b1f4d",direction:"ltr"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
        <div style={{fontWeight:800,fontSize:18}}>Sanitizer Concentration Verification Log — View (POS 19)</div>
          ...
          {record&&(<div ref={reportRef}>
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed",fontSize:12}}>
                <colgroup>
                  <col style={{width:110}}/><col style={{width:90}}/><col style={{width:160}}/><col style={{width:160}}/>
                ...
                <thead><tr>
                  <th style={thCell}>Date</th><th style={thCell}>Time</th><th style={thCell}>Sanitizer{"\n"}Type</th><th style={thCell}>Location /{ "\n"}Area</th>
