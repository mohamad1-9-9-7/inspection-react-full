// src/pages/monitor/branches/pos19/pos19_views/WoodenItemsConditionChecklistView.jsx
...
export default function WoodenItemsConditionChecklistView() {
  ...
  async function fetchRecord(d=date){
    ...
    setFooter({verifiedBy:match?.payload?.verifiedBy||"",revDate:match?.payload?.revDate||"",revNo:match?.payload?.revNo||""});setEditing(false);}
  ...
  async function saveEdit(){
    ...
    const rid=getId(record);const cleaned=editRows.filter(r=>Object.values(r).some(v=>String(v||"").trim()!==""));
  ...
  function exportJSON(){if(!record)return;const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify({type:TYPE,payload:record.payload},null,2)],{type:"application/json"}));a.download=`POS19_WoodenItems_${record?.payload?.reportDate||date}.json`;a.click();URL.revokeObjectURL(a.href);}
  ...
  async function exportPDF(){
    ...
    const drawH=()=>{pdf.setFillColor(233,240,255);pdf.rect(0,0,pageW,headerH,"F");pdf.setFont("helvetica","bold");pdf.setFontSize(13);pdf.text(`POS 19 | Wooden Items Condition Checklist (Weekly) — ${record?.payload?.reportDate||date}`,pageW/2,28,{align:"center"});};
    ...
    pdf.save(`POS19_WoodenItems_${record?.payload?.reportDate||date}.pdf`);
  }
      ...
      <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:12}}>
        ...
        <div>
          ...
          {record&&(<div ref={reportRef}>
            <ReportHeader
              title="Wooden Items Condition Checklist"
              fields={[
