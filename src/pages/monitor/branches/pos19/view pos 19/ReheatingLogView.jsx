// src/pages/monitor/branches/pos19/view pos 19/ReheatingLogView.jsx
...
const TYPE     = "pos19_reheating_log";
const BRANCH   = "POS 19";
const FORM_REF = "FS-HACCP/POS19/RH/01";
...
const emptyRow = () => ({ foodItem:"", batchNo:"", method:"", startTime:"", initialTemp:"", endTime:"", finalTemp:"", destination:"", correctiveAction:"" });

export default function ReheatingLogView() {
  ...
  async function fetchAllDates() {
    try { const uniq = await listReportDates(TYPE); setAllDates(uniq); if (!uniq.includes(date) && uniq.length) setDate(uniq[0]); } catch(e) { console.warn(e); }
  }
  async function fetchRecord(d=date) {
    setLoading(true); setErr(""); setRecord(null); setEditRows([]);
    try { const match = await getReportByDate(TYPE, d); setRecord(match); const rows=match?.payload?.entries??[]; setEditRows(rows.length?JSON.parse(JSON.stringify(rows)):[]); setEditing(false); } catch(e) { console.error(e); setErr("Failed to fetch data."); } finally { setLoading(false); }
  }
  ...
  function exportJSON() { if (!record) return; const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob([JSON.stringify({type:TYPE,payload:record.payload},null,2)],{type:"application/json"})); a.download=`POS19_Reheating_${record?.payload?.reportDate||date}.json`; a.click(); URL.revokeObjectURL(a.href); }
  ...
  async function exportXLSX() {
    try {
      ...
      const wb = new ExcelJS.Workbook(); const ws = wb.addWorksheet("Reheating");
      ...
      const COL_HEADERS = ["Food Item","Batch No","Method","Start Time","Initial °C","End Time","Final °C (≥75)","Destination","Corrective Action"];
      ws.columns = [{width:20},{width:12},{width:14},{width:12},{width:12},{width:12},{width:14},{width:16},{width:22}];
      ws.mergeCells(1,1,1,COL_HEADERS.length); const r1=ws.getCell(1,1); r1.value=`POS 19 | Reheating Log — ${FORM_REF}`; r1.alignment={horizontal:"center",vertical:"middle"}; r1.font={size:13,bold:true}; r1.fill={type:"pattern",pattern:"solid",fgColor:{argb:"E9F0FF"}}; ws.getRow(1).height=22;
      ...
      let i = 5; rows.forEach(e => { ws.getRow(i).values=[safe(e.foodItem),safe(e.batchNo),safe(e.method),safe(e.startTime),safe(e.initialTemp),safe(e.endTime),safe(e.finalTemp),safe(e.destination),safe(e.correctiveAction)]; ws.getRow(i).eachCell(c=>{c.alignment={horizontal:"center",vertical:"middle",wrapText:true};c.border=border;}); ws.getRow(i).height=20; i++; });
      const buf = await wb.xlsx.writeBuffer({useStyles:true,useSharedStrings:true});
      const a = document.createElement("a"); a.href=URL.createObjectURL(new Blob([buf],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"})); a.download=`POS19_Reheating_${p.reportDate||date}.xlsx`; a.click(); URL.revokeObjectURL(a.href);
  ...
  async function exportPDF() {
    ...
    const drawH = () => { pdf.setFillColor(233,240,255); pdf.rect(0,0,pageW,headerH,"F"); pdf.setFont("helvetica","bold"); pdf.setFontSize(13); pdf.text(`POS 19 | Reheating Log — ${record?.payload?.reportDate||date}`, pageW/2, 28, {align:"center"}); };
    ...
    pdf.save(`POS19_Reheating_${record?.payload?.reportDate||date}.pdf`);
  }
            ...
            <div style={{ overflowX:"auto", overflowY:"hidden" }}>
              <div ref={reportRef} style={{ width:"max-content" }}>
                ...
                <table style={gridStyle}>
                  <colgroup>
                    <col style={{width:170}}/><col style={{width:110}}/><col style={{width:120}}/>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th style={{...thCell, background:"#fef3c7"}}>Start Time</th>
                      <th style={{...thCell, background:"#fef3c7"}}>Initial °C</th>
                      <th style={{...thCell, background:"#dcfce7"}}>End Time</th>
                      <th style={{...thCell, background:"#dcfce7"}}>Final °C (≥75)</th>
                      <th style={thCell}>Destination</th>
                  ...
                  <tbody>
                    {!editing ? rows.filter(isFilledRow).map((r, i) => (
                      <tr key={i}>
                        ...
                        <td style={tdCell}>{safe(r.method)}</td>
                        <td style={tdCell}>{safe(r.startTime)}</td>
                        <td style={tdCell}>{safe(r.initialTemp)}</td>
                        <td style={tdCell}>{safe(r.endTime)}</td>
                        <td style={{...tdCell, ...(lowFinal(r.finalTemp)?{background:"#fef2f2",color:"#b91c1c",fontWeight:700}:{})}}>{safe(r.finalTemp)}</td>
                      ...
                      <tr key={i}>
                        ...
                        <td style={tdCell}><input value={r.method||""} onChange={e=>upd(i,"method",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="time" value={r.startTime||""} onChange={e=>upd(i,"startTime",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="number" step="0.1" value={r.initialTemp||""} onChange={e=>upd(i,"initialTemp",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="time" value={r.endTime||""} onChange={e=>upd(i,"endTime",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="number" step="0.1" value={r.finalTemp||""} onChange={e=>upd(i,"finalTemp",e.target.value)} style={inputStyle}/></td>
                ...
                <div style={{ marginTop:10, paddingTop:8, borderTop:"2px solid #1f3b70", fontSize:12, color:"#0b1f4d", lineHeight:1.6, width:"max-content" }}>
                  <strong>Note (CCP):</strong> Final temperature must reach ≥75°C for at least 15 seconds. Reheat only once.
                </div>
                ...
                <div style={{ marginTop:12, width:"100%", display:"flex", justifyContent:"space-between", gap:16, flexWrap:"wrap", alignItems:"center", fontSize:12 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, flex:"1 1 320px", minWidth:300 }}><strong>Checked by:</strong><span style={{ display:"inline-block", minWidth:260, borderBottom:"2px solid #1f3b70", lineHeight:"1.8" }}><SignatureName name={safe(record.payload?.checkedBy)} underline={false} /></span></div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, flex:"1 1 320px", minWidth:300, justifyContent:"flex-end" }}><strong>Verified by:</strong><span style={{ display:"inline-block", minWidth:260, borderBottom:"2px solid #1f3b70", lineHeight:"1.8" }}><SignatureName name={safe(record.payload?.verifiedBy)} underline={false} /></span></div>
                </div>
              </div>
            </div>
          )}
