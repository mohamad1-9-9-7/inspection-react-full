// src/pages/monitor/branches/pos19/view pos 19/DefrostingRecordView.jsx
...
export default function DefrostingRecordView() {
  ...
  function addRow() { setEditRows(p => [...p, { rawMaterial:"", quantity:"", brand:"", rmProdDate:"", rmExpDate:"", defStartDate:"", defStartTime:"", startTemp:"", defEndDate:"", defEndTime:"", endTemp:"", defrostTemp:"", remarks:"" }]); }
  ...
  function exportJSON() {
    ...
    a.download = `POS19_Defrosting_${record?.payload?.reportDate || date}.json`;
    a.click(); URL.revokeObjectURL(a.href);
  }
  async function importJSON(file) {
    if (!file) return;
    try {
      const txt = await file.text(); const parsed = JSON.parse(txt); const payload = parsed?.payload || parsed;
  ...
  async function exportXLSX() {
    try {
      ...
      const wb = new ExcelJS.Workbook(); const ws = wb.addWorksheet("Defrosting");
      ...
      const COL_HEADERS = ["Item","Qty","Brand","RM Prod.","RM Exp.","Start Date","Start Time","Start °C","End Date","End Time","End °C","Chiller °C","Remarks"];
      ws.columns = [{width:18},{width:10},{width:14},{width:13},{width:13},{width:13},{width:11},{width:10},{width:13},{width:11},{width:10},{width:11},{width:22}];
      ws.mergeCells(1,1,1,COL_HEADERS.length);
      const r1 = ws.getCell(1,1); r1.value = `POS 19 | Defrosting Record — ${FORM_REF}`;
      ...
      let i = 5;
      rows.forEach(e => {
        ws.getRow(i).values = [safe(e.rawMaterial), safe(e.quantity), safe(e.brand), safe(e.rmProdDate), safe(e.rmExpDate), safe(e.defStartDate), safe(e.defStartTime), safe(e.startTemp), safe(e.defEndDate), safe(e.defEndTime), safe(e.endTemp), safe(e.defrostTemp), safe(e.remarks)];
      ...
      a.download = `POS19_Defrosting_${p.reportDate || date}.xlsx`; a.click(); URL.revokeObjectURL(a.href);
  ...
  async function exportPDF() {
    ...
    const drawH = () => { pdf.setFillColor(233,240,255); pdf.rect(0,0,pageW,headerH,"F"); pdf.setFont("helvetica","bold"); pdf.setFontSize(13); pdf.text(`POS 19 | Defrosting Record — ${record?.payload?.reportDate || date}`, pageW/2, 28, { align:"center" }); };
    ...
    pdf.save(`POS19_Defrosting_${record?.payload?.reportDate || date}.pdf`);
  }
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #dbe3f4", borderRadius:12, padding:16, color:"#0b1f4d", direction:"ltr" }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
        <div style={{ fontWeight:800, fontSize:18 }}>Defrosting Record — View (POS 19)</div>
          ...
          {record && (
            <div style={{ overflowX:"auto", overflowY:"hidden" }}>
              <div ref={reportRef} style={{ width:"max-content" }}>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:8, marginBottom:8, fontSize:12, minWidth:1400 }}>
                ...
                <table style={gridStyle}>
                  <colgroup>
                    <col style={{width:160}}/><col style={{width:90}}/><col style={{width:130}}/><col style={{width:120}}/><col style={{width:120}}/>
                  ...
                  <thead>
                    <tr>
                      <th colSpan={5} style={{...thCell, background:"#e0f2fe"}}>Raw Material Details</th>
                      <th colSpan={3} style={{...thCell, background:"#dbeafe"}}>Defrost START</th>
                      <th colSpan={3} style={{...thCell, background:"#ede9fe"}}>Defrost END</th>
                      <th colSpan={2} style={{...thCell, background:"#dcfce7"}}>Result</th>
                      {editing && <th rowSpan={2} style={thCell}>—</th>}
                    </tr>
                    <tr>
                      <th style={thCell}>Item</th><th style={thCell}>Qty</th><th style={thCell}>Brand</th><th style={thCell}>Prod.{"\n"}Date</th><th style={thCell}>Exp.{"\n"}Date</th>
                      <th style={thCell}>Date</th><th style={thCell}>Time</th><th style={thCell}>°C</th>
                      <th style={thCell}>Date</th><th style={thCell}>Time</th><th style={thCell}>°C</th>
                      <th style={thCell}>Chiller °C</th><th style={thCell}>Remarks</th>
                    </tr>
                  </thead>
                  <tbody>
                    {!editing ? rows.filter(isFilledRow).map((r, i) => (
                      <tr key={i}>
                        <td style={tdCell}>{safe(r.rawMaterial)}</td><td style={tdCell}>{safe(r.quantity)}</td><td style={tdCell}>{safe(r.brand)}</td>
                        <td style={tdCell}>{formatDMY(safe(r.rmProdDate))}</td><td style={tdCell}>{formatDMY(safe(r.rmExpDate))}</td>
                        <td style={tdCell}>{formatDMY(safe(r.defStartDate))}</td><td style={tdCell}>{safe(r.defStartTime)}</td><td style={tdCell}>{safe(r.startTemp)}</td>
                        <td style={tdCell}>{formatDMY(safe(r.defEndDate))}</td><td style={tdCell}>{safe(r.defEndTime)}</td><td style={tdCell}>{safe(r.endTemp)}</td>
                        <td style={tdCell}>{safe(r.defrostTemp)}</td><td style={tdCell}>{safe(r.remarks)}</td>
                      </tr>
                    )) : editRows.map((r, i) => (
                      <tr key={i}>
                        <td style={tdCell}><input value={r.rawMaterial||""} onChange={e=>upd(i,"rawMaterial",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input value={r.quantity||""} onChange={e=>upd(i,"quantity",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input value={r.brand||""} onChange={e=>upd(i,"brand",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="date" value={r.rmProdDate||""} onChange={e=>upd(i,"rmProdDate",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="date" value={r.rmExpDate||""} onChange={e=>upd(i,"rmExpDate",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="date" value={r.defStartDate||""} onChange={e=>upd(i,"defStartDate",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="time" value={r.defStartTime||""} onChange={e=>upd(i,"defStartTime",e.target.value)} style={inputStyle}/></td>
                        ...
                        <td style={tdCell}><input type="date" value={r.defEndDate||""} onChange={e=>upd(i,"defEndDate",e.target.value)} style={inputStyle}/></td>
                        <td style={tdCell}><input type="time" value={r.defEndTime||""} onChange={e=>upd(i,"defEndTime",e.target.value)} style={inputStyle}/></td>
                ...
                <div style={{ marginTop:10, paddingTop:8, borderTop:"2px solid #1f3b70", fontSize:12, color:"#0b1f4d", lineHeight:1.6, width:"max-content" }}>
                  <strong>Note:</strong> Defrost at ≤ 5°C inside chiller. Use within 24 hours of defrosting.
                </div>
                ...
                <div style={{ marginTop:12, width:"100%", display:"flex", justifyContent:"space-between", gap:16, flexWrap:"wrap", alignItems:"center", fontSize:12 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, flex:"1 1 320px", minWidth:300 }}>
                    ...
                    <span style={{ display:"inline-block", minWidth:260, borderBottom:"2px solid #1f3b70", lineHeight:"1.8", textAlign:"left" }}><SignatureName name={safe(record.payload?.checkedBy)} underline={false} /></span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, flex:"1 1 320px", minWidth:300, justifyContent:"flex-end" }}>
                    <strong>Verified by:</strong>
                    <span style={{ display:"inline-block", minWidth:260, borderBottom:"2px solid #1f3b70", lineHeight:"1.8", textAlign:"left" }}><SignatureName name={safe(record.payload?.verifiedBy)} underline={false} /></span>
                  </div>
                </div>
              </div>
            </div>
