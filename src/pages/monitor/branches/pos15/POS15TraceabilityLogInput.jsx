// src/pages/monitor/branches/pos15/POS15TraceabilityLogInput.jsx
...
/* ===== (NEW) Key normalizer + collectors (handles keys with spaces) ===== */
...
/* ===== اشتقاق تواريخ الذبح/الصلاحية (REPLACED) ===== */
...
function normalizeRawForIndex(doc) {
  ...
  return {
    ...
    awb: String(awb || "").trim(),
    invoice: String(invoice || "").trim(),              // <— NEW
...
/* ===== المكون الرئيسي ===== */
export default function POS15TraceabilityLogInput() {
  /* ===== ترويسة ===== */
  ...
  const [activeBatch, setActiveBatch] = useState(null); // index of batch to fill
  ...
  const filteredRaw = useMemo(() => {
    ...
    return rawIndex.filter(r => {
      const hay = [r.awb, r.invoice, r.shipment, r.slaughterDate, r.expiryDate]  // <— include invoice in search
        .join(" ")
        .toLowerCase();
    ...
    <div style={{ background:"#fff", border:"1px solid #dbe3f4", borderRadius:12, padding:16, color:"#0b1f4d" }}>
          ...
          <tbody>
            {batches.map((b, bi) => (
              <tr key={bi}>
                ...
                {/* Batch ID + RAW Modal trigger */}
                <td style={{ ...tdCell, textAlign:"left" }}>
                  <div style={{ display:"grid", gap:6 }}>
                    <input
                      type="text"
                      placeholder="e.g., B-20251023-001 or AWB"
                      value={b.batchId}
      ...
      >
        ...
        <span style={{ marginInlineStart: 4 }}>
          This form uses a single row per Batch / Lot, with dynamic raw inputs and final outputs.
          Data is flattened to the legacy entries format on save for backend compatibility.
        </span>
      </div>
