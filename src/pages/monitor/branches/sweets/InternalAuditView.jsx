...
const TYPE = "sweets_internal_audit";
...
export default function InternalAuditView() {
  ...
  const fetchRecords = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const res = await fetch(`${API_BASE}/api/reports?reporter=sweets&type=${TYPE}`);
  ...
  if (!selected) {
    return (
      <div style={{ fontFamily: "'Inter', system-ui, sans-serif", padding: 24, maxWidth: "100%", margin: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748b" }}>Internal Audit</p>
          </div>
          <button
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 900, color: "#0f172a" }}>
              {hTop.documentTitle || "INTERNAL AUDIT"}
            </h2>
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#64748b" }}>
              {hTop.documentNo} · Issue No: {hTop.issueNo} · Revision: {hTop.revisionNo} · Issue Date: {hTop.issueDate || ""}
            </p>
          </div>
          {sc?.score != null && (
