...
//   type = sweets_coolers_config , payload.reportDate = "config"
...
export const COOLERS_CONFIG_CACHE_KEY = "sweets_coolers_config_cache_v1";

/** How many storage units the sheet has. Sweets: 5 coolers + 1 freezer. */
export const COOLER_COUNT = 6;
...
export function defaultCoolerDef(index) {
  // Sweets: last unit is the freezer, the rest are chillers (Cooler 1..5).
  if (index === COOLER_COUNT - 1) return { type: "frozen", label: "FREEZER", min: -19, max: -14 };
...
export async function saveCoolerConfig(coolerDefs, loadingDef, user = "") {
  const body = {
    reporter: user || "sweets-coolers-config",
