...
async function uploadImageViaServer(file) {
  ...
  if (!res.ok || !data.ok || !(data.optimized_url || data.url)) {
    const txt = typeof data === "object" ? JSON.stringify(data) : await res.text().catch(() => "");
...
async function deleteImageUrl(url) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !data?.ok) throw new Error(`Delete failed ${res.status}: ${data?.error || ""}`);
  return true;
}
...
const DEFAULTS_MIXED = {
  fat: "NIL", discolour: "NIL", dehydration: "NIL", whitePatches: "NIL", brokenCut: "NIL",
  appearance: "OK", parasites: "NIL", filth: "NIL", boneCartilage: "NIL", bloodSpot: "NIL",
  feather: "NIL", skin: "NIL", texture: "OK", badOdour: "NIL",
};
const DEFAULTS_GRILLER = {
  fat: "OK", discolour: "NIL", dehydration: "NIL", odour: "NIL", appearance: "OK", parasites: "NIL",
  brokenLeg: "NIL", yellowishLegSkin: "NIL", bloodSpot: "NIL", darkGreySkinLeg: "NIL",
  bruisesWingsSurface: "NIL", bruisesLegSurface: "NIL", featherSkin: "NIL",
  brokenWings: "NIL", neckSkin: "NIL", brokenSkin: "NIL",
};
const DEFAULTS_LIVER = {
  sampleNo: "1", sizeGrade: "NIL", sifNo: "NIL", lotNo: "NIL",
  fat: "NIL", discolour: "NIL", dehydration: "NIL", whitePatches: "NIL", brokenCut: "NIL",
  appearance: "OK", parasites: "NIL", filth: "NIL", boneCartilage: "NIL", bloodSpot: "NIL",
  feather: "NIL", skin: "NIL", texture: "OK", badOdour: "NIL",
};
const rowsFor = (v) => (v === "griller" ? ROWS_GRILLER : v === "liver" ? ROWS_LIVER : ROWS_MIXED);
const defaultsFor = (v) => (v === "griller" ? DEFAULTS_GRILLER : v === "liver" ? DEFAULTS_LIVER : DEFAULTS_MIXED);
...
export default function FreshChickenInter() {
  ...
  const [entryDates, setEntryDates] = useState({ mixed_parts: "", griller: "", liver: "" });
  const setEntryDate = (variant, value) => setEntryDates((p) => ({ ...p, [variant]: value }));
  ...
  const label = { fontWeight: 900, fontSize: 13, marginBottom: 6, display: "block" };
  const input = { width: "100%", maxWidth: "100%", padding: "10px 12px", borderRadius: 10, border: `2px solid ${COLORS.line}`, background: COLORS.card, color: COLORS.ink, fontSize: 14, boxSizing: "border-box" };
  const grid = (n) => ({ display: "grid", gridTemplateColumns: `repeat(${n}, minmax(0, 1fr))`, gap: 12 });
  const btn = { padding: "12px 18px", borderRadius: 10, border: `2px solid ${COLORS.line}`, background: COLORS.tabOn, color: "#fff", fontWeight: 900, cursor: "pointer" };
  const btnGhost = { padding: "12px 16px", borderRadius: 10, border: `2px dashed ${COLORS.line}`, background: COLORS.card, color: COLORS.ink, fontWeight: 800, cursor: "pointer" };
  const tabsWrap = { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, width: "100%" };
  const tabBtn = (active) => ({ width: "100%", padding: "14px 18px", borderRadius: 12, border: `2px solid ${COLORS.line}`, background: active ? COLORS.tabOn : COLORS.tabOff, color: active ? "#fff" : COLORS.ink, fontWeight: 900, cursor: "pointer" });
  const tableWrap = { overflowX: "auto", border: `2px solid ${COLORS.line}`, borderRadius: 12, background: COLORS.card };
  const table = { width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 14, tableLayout: "fixed" };
  ...
  const td = { borderTop: `2px solid ${COLORS.line}`, borderRight: `2px solid ${COLORS.line}`, padding: 8, verticalAlign: "top", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" };
  const tdAttr = { ...td, fontWeight: 900, background: "#fff", position: "sticky", left: 0, zIndex: 2, minWidth: 240 };

  /* ===== UI helpers ===== */
  const setH = (k, v) => setHeader((p) => ({ ...p, [k]: v }));
  const clampNonNegative = (val) => { const n = Number(val); if (!Number.isFinite(n)) return ""; return n < 0 ? 0 : n; };
  const clampTemperature = (val) => { const n = Number(val); if (!Number.isFinite(n)) return ""; return Math.min(15, Math.max(-5, n)); };
  ...
  const onDeleteImg = async (url, from = "images") => {
    try { await deleteImageUrl(url); } catch {} finally {
      const setters = { images: setImages, certs: setCerts }; const setFn = setters[from] || setImages;
    ...
    <div style={page} dir="ltr">
      <div style={container}>
        ...
        <div style={sectionCard}>
          ...
          <div style={tabsWrap}>
            {REPORT_TYPES.map((t) => (
              <button key={t.id} onClick={() => setReportVariant(t.id)} style={tabBtn(reportVariant === t.id)} type="button">
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr auto", gap: 10 }}>
            <div>
              ...
              <input type="date" style={input} value={entryDates[reportVariant] || ""} onChange={(e)=>setEntryDate(reportVariant, e.target.value)} />
            </div>
        ...
        <div style={sectionCard}>
          ...
          <div style={grid(5)}>
            <div><label style={label}>Report No</label><input style={input} value={header.reportNo} onChange={(e)=>setH("reportNo",e.target.value)} placeholder="e.g., 1" /></div>
            <div><label style={label}>Sample Received On</label><input type="date" style={input} value={header.sampleReceivedOn} onChange={(e)=>setH("sampleReceivedOn", e.target.value)} /></div>
            <div><label style={label}>Inspection Date</label><input type="date" style={input} value={header.inspectionDate} onChange={(e)=>setH("inspectionDate", e.target.value)} /></div>
            <div>
              <label style={label}>Truck / Product Temperature (°C)</label>
              <input type="number" step="0.1" min={-5} max={15} style={input}
                     value={header.truckTemperature} onChange={(e)=>setH("truckTemperature", String(clampTemperature(e.target.value)))} placeholder="e.g., 4.6" />
              {tempBadge && (
                <div style={{ marginTop: 6, fontWeight: 800, fontSize: 12, display: "inline-block", padding: "6px 10px", borderRadius: 8, border: `2px solid ${COLORS.line}`, background: "#fff", color: tempBadge.color }}>
                  {tempBadge.text}
                </div>
              )}
            </div>
            <div><label style={label}>Brand</label><input style={input} value={header.brand} onChange={(e)=>setH("brand",e.target.value)} placeholder="e.g., AL AIN FARM (AL REEF)" /></div>
            <div><label style={label}>Invoice Number</label><input style={input} value={header.invoiceNumber} onChange={(e)=>setH("invoiceNumber",e.target.value)} placeholder="e.g., KMP 20215" /></div>
            <div><label style={label}>Origin</label><input style={input} value={header.origin} onChange={(e)=>setH("origin",e.target.value)} placeholder="Locally Slaughtered" /></div>
            <div><label style={label}>Supplier</label><input style={input} value={header.supplier} onChange={(e)=>setH("supplier",e.target.value)} placeholder="e.g., KANZ AL MADAM" /></div>
            <div style={{ gridColumn: "1 / -1" }}><label style={label}>Product Name</label><input style={input} value={header.productName} onChange={(e)=>setH("productName",e.target.value)} placeholder={defaultProductName} /></div>
          </div>
        </div>
        ...
        <div style={sectionCard}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginBottom: 8 }}>
            <button type="button" style={btnGhost} onClick={addSampleColumn}>+ Add Sample (column)</button>
            <button type="button" style={{ ...btnGhost, borderStyle: "solid" }} onClick={removeLastSampleColumn}>Remove Sample (column)</button>
          </div>
          ...
          <div style={tableWrap} dir="ltr">
            <table style={table}>
              <colgroup>
                <col style={{ width: 260 }} />
                {currentSamples.map((_, i) => (<col key={i} style={{ width: 220 }} />))}
              </colgroup>
              <thead>
                <tr>
                  <th style={{ ...th, left: 0, position: "sticky", zIndex: 2, borderRight: `2px solid ${COLORS.line}` }}>Attribute</th>
                  {currentSamples.map((s, i) => (<th key={i} style={th}>{`Sample ${i + 1}`}</th>))}
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={row.key}>
                    ...
                    {currentSamples.map((s, colIdx) => (
                      <td key={`${row.key}-${colIdx}`} style={td}>
                        {row.type === "number" ? (
                          <input type="number" step="0.1" style={input} value={s[row.key] ?? ""} onChange={(e)=>setCell(colIdx,row.key,e.target.value,"number")} />
                        ) : (
                          <input style={input} value={s[row.key] ?? ""} onChange={(e)=>setCell(colIdx,row.key,e.target.value,"text")} />
                        )}
                      </td>
                    ))}
                  </tr>
        ...
        <div style={sectionCard}>
          ...
          <textarea rows={3} style={{ ...input, resize: "vertical", whiteSpace: "pre-wrap" }} value={remarks} onChange={(e)=>setRemarks(e.target.value)} />
        </div>
        ...
        <div style={sectionCard}>
          ...
          <div style={{ ...grid(2) }}>
            {/* Images */}
            <div onDragEnter={prevent} onDragOver={prevent} onDrop={onDropZone("images")} style={{ padding: 14, border: `2px dashed ${COLORS.line}`, borderRadius: 12, background: "#fff" }}>
              ...
              <div style={{ fontSize: 12, opacity: .8, marginBottom: 8 }}>Drag & drop images here or use “Choose images”.</div>
        ...
        <div style={sectionCard}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><label style={label}>CHECKED BY:</label><input style={input} value={checkedBy} onChange={(e)=>setCheckedBy(e.target.value)} placeholder="Name / Signature" /></div>
            <div><label style={label}>VERIFIED BY:</label><input style={input} value={verifiedBy} onChange={(e)=>setVerifiedBy(e.target.value)} placeholder="Name / Signature" /></div>
          </div>
        </div>
      </div>
    </div>
...
function ThumbGrid({ items = [], onDelete, docMode = false }) {
  ...
  const delBtn = { position: "absolute", top: 6, left: 6, padding: "6px 10px", fontWeight: 900, borderRadius: 8, border: "2px solid #0b132b", background: "#fff", cursor: "pointer" };
    ...
    <div style={grid}>
      {items.map((it, i) => (
        <div style={cell} key={it.url || i}>
          <button type="button" onClick={()=>onDelete(it.url)} style={delBtn}>Delete</button>
          {docMode ? (
            <a href={it.url} target="_blank" rel="noreferrer" style={{ fontWeight: 800, textDecoration: "underline" }}>Open document</a>
          ) : (
      ...
      {items.length === 0 && (
        <div style={{ gridColumn: "1 / -1", fontSize: 12, opacity: .7, textAlign: "center" }}>
          No attachments yet.
        </div>
      )}
    </div>
