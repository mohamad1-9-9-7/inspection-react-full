...
// تصغير الصورة قبل التحويل Base64 (≈1280px الأطول و ~80% جودة)
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        try {
          ...
          resolve(reader.result); // fallback
        }
      };
  ...
  async function checkDuplicateForDay(dateStr) {
    try {
      ...
      for (const it of items) {
        ...
        if (d === dateStr && site === FIXED_AREA && branch === BRANCH) {
          return true;
        }
      }
      return false;
  ...
  return (
    <div style={sheet}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1, textAlign: "center" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 12 }}>
              MEAT PRODUCT INSPECTION REPORT — MAMZAR PARK (FTR 2)
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 14 }}>
        <button onClick={handleSave} style={{ ...btn("#2563eb"), opacity: saving ? 0.6 : 1, pointerEvents: saving ? "none" : "auto" }}>
          {saving ? "Saving…" : "💾 Save"}
        </button>
      </div>
      ...
      {modalState.open && (
        <div style={overlay} onClick={closeModal}>
          <div style={modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginTop: 10, fontWeight: 800, color: "#0b1f4d" }}>
              {modalState.text}
            </div>
