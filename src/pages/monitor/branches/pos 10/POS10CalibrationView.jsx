// src/pages/monitor/branches/pos 10/POS10CalibrationView.jsx
...
export default function POS10CalibrationView() {
  const TYPE = "pos10_calibration_log";
  ...
  // Group by Year → Month (latest first)
  ...
  const handleExportPDF = async () => {
    ...
    pdf.text("AL MAWASHI — POS 10 Calibration Log", pageWidth / 2, 28, {
      align: "center",
    });
  ...
  const handleExportJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `POS10_Calibration_ALL_${ts}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
  ...
  return (
    <div style={{ display: "flex", gap: "1rem" }}>
      ...
      >
        <h4 style={{ marginBottom: 12, color: "#6d28d9", textAlign: "center" }}>
                      ...
                      return (
                        <details key={m} style={{ marginLeft: "1rem" }}>
                          <summary style={{ fontWeight: 600 }}>
                            📅 Month {m}{" "}
                            <span style={{ color: "#6b7280" }}>({arr.length})</span>
                          </summary>
              ...
              >
                <tbody>
                  <tr>
                    ...
                    >
                      <div
                        style={{ fontWeight: 900, color: "#a00", lineHeight: 1.1 }}
                      >
                        AL
                        <br />
                        MAWASHI
                  ...
                  <tr>
                    ...
                    <td style={tdHeader}>
                      <b>Date:</b> {selected?.payload?.reportDate || "—"}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              >
                <thead>
                  <tr>
                    ...
                    <th style={thCell}>Calib. Date</th>
                    <th style={thCell}>Next Due</th>
                  </tr>
                </thead>
                <tbody>
                  {selected?.payload?.entries?.map((row, idx) => {
                    const filename = `POS10_Calibration_${selected?.payload?.reportDate || "date"}_${idx + 1}.png`;
                    return (
                      <tr key={idx}>
            ...
            <div style={lightboxFooter}>
              <button
                ...
                onClick={() =>
                  downloadBase64(
                    preview.src,
                    `POS10_Calibration_Image_${(selected?.payload?.reportDate || "date")}_${(preview.idx ?? 0) + 1}.png`
                  )
                }
              >
                ⬇ Download
