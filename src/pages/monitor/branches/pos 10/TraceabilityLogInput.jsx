...
function deriveSlaughterAndExpiry(payloadIn) {
  ...
  [payload, header, gi].forEach((o) => {
    _valuesByAliases(o, SLAUGHTER_KEYS).forEach((v) =>
      extractAllDates(v).forEach((d) => sDates.push(d))
    );
    _valuesByAliases(o, EXPIRY_KEYS).forEach((v) =>
      extractAllDates(v).forEach((d) => eDates.push(d))
    );
  });
  const rows =
    payload.rows ||
  ...
  if (Array.isArray(rows))
    rows.forEach((r) => {
      _valuesByAliases(r, SLAUGHTER_KEYS).forEach((v) =>
        extractAllDates(v).forEach((d) => sDates.push(d))
      );
      _valuesByAliases(r, EXPIRY_KEYS).forEach((v) =>
        extractAllDates(v).forEach((d) => eDates.push(d))
      );
    });
  (Array.isArray(payload.samples) ? payload.samples : []).forEach(
    (s) => {
      _valuesByAliases(s, SLAUGHTER_KEYS).forEach((v) =>
        extractAllDates(v).forEach((d) => sDates.push(d))
      );
      _valuesByAliases(s, EXPIRY_KEYS).forEach((v) =>
        extractAllDates(v).forEach((d) => eDates.push(d))
      );
    }
  );
  return {
    slaughterDate: formatDateList(
      Array.from(new Set(sDates)).sort()
    ),
...
function normalizeRawForIndex(doc) {
  ...
  const { slaughterDate, expiryDate } = deriveSlaughterAndExpiry(
    payload
  );
...
async function fetchRawIndex({ signal } = {}) {
  ...
  const items = Array.isArray(data)
    ? data
    : (data && (data.data || data.items || data.reports)) || [];
...
export default function TraceabilityLogInput() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setRawIndex(await fetchRawIndex({ signal: ac.signal }));
      } catch (e) {
  ...
  const deleteBatch = (bi) =>
    setBatches((prev) => prev.filter((_, i) => i !== bi));
  const duplicateBatch = (bi) =>
    setBatches((prev) => {
  ...
  const addOutput = (bi) =>
    setBatches((prev) => {
      ...
      next[bi] = {
        ...next[bi],
        outputs: [...next[bi].outputs, emptyOutput()],
      };
      return next;
    });
  const deleteOutput = (bi, oi) =>
    setBatches((prev) => {
      const next = [...prev];
      ...
      next[bi] = {
        ...next[bi],
        outputs: arr.length ? arr : [emptyOutput()],
      };
      return next;
    });
  ...
  async function handleSave() {
    ...
    if (hasExistingForDate) {
      alert(
        "⚠️ تم العثور على تقرير محفوظ لنفس التاريخ لهذا الفرع.\n" +
          "A report is already saved for this date for this branch.\n" +
          "لا يمكن حفظ تقرير جديد لنفس اليوم. يرجى تغيير التاريخ أو مراجعة التقرير من شاشة التقارير."
      );
      return;
    }
    ...
    if (!hasAnyData) {
      alert(
        "الرجاء تعبئة دفعة واحدة على الأقل بمادة أولية أو منتج نهائي قبل الحفظ."
      );
      return;
    }
    if (!checkedBy.trim()) {
      alert("حقل Checked by إلزامي.");
      return;
    }
    if (!verifiedBy.trim()) {
      alert("حقل Verified by إلزامي.");
      return;
    }
    ...
    const cleaned = entries.filter((r) =>
      rowKeys.some(
        (k) => (r[k] ?? "").toString().trim() !== ""
      )
    );
  ...
  return (
    ...
    >
      ...
      {hasExistingForDate && (
        ...
        >
          ⚠️ تم العثور على تقرير محفوظ لنفس التاريخ لهذا
          الفرع. لا يمكن حفظ تقرير جديد لنفس اليوم.
        </div>
      )}
