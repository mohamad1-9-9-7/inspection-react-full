...
/* ===== Draft (localStorage) ===== */
const DRAFT_KEY = "pos10_temperature_draft_v1";
const loadDraft = () => {
  try {
...
// Times ("Corrective Action" is kept in the payload for compatibility, not shown as a column)
const times = [
  "8:00 AM",
  "11:00 AM",
  "2:00 PM",
...
export default function POS10TemperatureInput() {
  ...
  const addPV = (storageKey) =>
    setProductVerifications((prev) => [...prev, makePV(storageKey, DEFAULT_MATCH_TIME)]);
  ...
  const handleSave = async () => {
    const d = toISODate(reportDate);
    if (!d) return alert("⚠️ Please choose the report date first.");
    ...
    if (dateBusy) return alert("⏳ Please wait — checking the date…");
    if (dateTaken) {
    ...
    try {
      ...
      if (existsNow) {
        setDateTaken(true);
        setOpMsg("");
        return alert("⛔ A report already exists for this day.\nChoose another date or edit the existing report.");
      }
    ...
    <div style={{ background: "linear-gradient(120deg, #f6f8fa 65%, #e8daef 100%)", padding: "1.5rem", borderRadius: "14px", boxShadow: "0 4px 18px #d2b4de44" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, marginTop: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={handleSave} style={{ ...saveBtn, opacity: disableSave ? 0.6 : 1, pointerEvents: disableSave ? "none" : "auto" }} title={disableSave ? (dateBusy ? "Checking the date..." : "Only one report per day is allowed") : "Save to Server"}>
            💾 Save to Server
          </button>
        </div>
      </div>
