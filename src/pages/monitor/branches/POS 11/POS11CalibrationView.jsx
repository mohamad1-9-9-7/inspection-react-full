...
export default function POS11CalibrationView() {
  ...
  async function exportXLSX() {
    try {
      ...
      const xlsxRows = Array.isArray(p.entries) ? p.entries.filter(isFilledRow) : [];
      ...
      console.error("[XLSX export error]", xlsxErr);
      try {
        const p = record?.payload || {};
        fallbackCSV(p);
        alert("⚠️ XLSX export failed, CSV exported instead.\n" + (xlsxErr?.message || xlsxErr));
      } catch (e2) {
  ...
  return (
    <GlassShell
      icon="📐"
      title="Calibration Log — View (POS 11)"
      actions={
        <>
            ...
            <div ref={reportRef} style={{ width: "100%", minWidth: 0 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 8, marginBottom: 10, fontSize: 14.5 }}>
                {[
                  ...
                  ["Total Items", String(record.payload?.entries?.filter?.(isFilledRow)?.length || 0)],
                ].map(([k, v]) => (
                  <div key={k} style={{
