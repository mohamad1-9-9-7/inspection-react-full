...
export default function POS11PestControlInput() {
  const TYPE = "pos11_pest_control"; // ✅ لفرع POS 11 (ملحمة العين)

  const [form, setForm] = useState({
    reportDate: new Date().toISOString().slice(0, 10),
    branch: "POS 11", // ✅ فرع افتراضي
    serviceProvider: "",
  ...
  async function handleSave() {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/reports`, {
        ...
        body: JSON.stringify({ reporter: "pos11", type: TYPE, payload: form }),
      });
  ...
  return (
    <div style={wrap}>
      ...
      >
        PEST CONTROL — {form.branch || "POS 11"}
      </div>
