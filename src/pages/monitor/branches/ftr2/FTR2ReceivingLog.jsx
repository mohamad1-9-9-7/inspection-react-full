...
export default function FTR2ReceivingLog() {
  ...
  // Toast state
  const [toast, setToast] = useState({ show: false, text: "", type: "info" }); // type: 'info' | 'success' | 'error'
  ...
  async function handleSave() {
    ...
    if (entries.length === 0) { showToast("لا يوجد بيانات للحفظ.", "error", 3000); return; }
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontWeight:800, fontSize:16 }}>
            Receiving Log <span style={{ fontWeight:600 }}>(Food Truck – FTR 2)</span>
          </div>
        </div>
