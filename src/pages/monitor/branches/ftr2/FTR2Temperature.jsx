...
/* ===== Draft (localStorage) ===== */
const DRAFT_KEY = "ftr2_temperature_draft_v1";
const loadDraft = () => {
  try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/reports`, {
        ...
        body: JSON.stringify({ reporter: "ftr2", type: "ftr2_temperature", payload }),
      });
        ...
        <tbody>
          {coolers.map((c, row) => (
            <tr key={row}>
              ...
              {/* Remarks — تُحفظ كذلك داخل temps['Corrective Action'] */}
              <td style={tdCellLeft}>
                <input
                  type="text"
                  value={c.remarks}
